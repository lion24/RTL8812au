...
)
{
	...
	if (p_dm_odm->is_linked) {
		...
		if (p_dm_odm->hp_hw_id && (p_dm_odm->traffic_load == TRAFFIC_ULTRA_LOW))
...
)
{
...
#endif
	u1Byte					rssi_L2H = 43, rssi_H2L = 37;
	BOOLEAN					equal_weighting_en = 0;
	...
	if (p_dm_odm->SupportICType & ODM_RTL8197F) {
		/* force AGC weighting */
		odm_set_bb_reg(p_dm_odm, 0xc54, BIT0, 1);
		/* MRC by AGC table */
		odm_set_bb_reg(p_dm_odm, 0xce8, BIT30, 1);
		/* disable antenna_weighting_shift mechanism */
		odm_set_bb_reg(p_dm_odm, 0xd5c, BIT29, 0);
		/* use modified ZF eqz */
		odm_set_bb_reg(p_dm_odm, 0xd18, BIT23, 1);

		if (p_dm_odm->rssi_min_by_path != 0xFF) {

			if (p_dm_odm->rssi_min_by_path >= rssi_L2H) {
				/* equal weighting */
				odm_set_bb_reg(p_dm_odm, 0xd5c, BIT31 | BIT30, 0);
				equal_weighting_en = 1;

			} else if (p_dm_odm->rssi_min_by_path <= rssi_L2H) {

				/* AGC weighting */
				odm_set_bb_reg(p_dm_odm, 0xd5c, BIT31 | BIT30, 1);
			}
		} else {
			/* AGC weighting */
			odm_set_bb_reg(p_dm_odm, 0xd5c, BIT31 | BIT30, 1);
...
/*
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_ANT_WEIGHTING, ODM_DBG_LOUD, ("rssi_min_by_path = %d, equal_weighting_en = %d\n",
		 p_dm_odm->rssi_min_by_path,
		 equal_weighting_en));
*/
	} else {
		return;
