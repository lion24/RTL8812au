...
)
{
	...
	odm_memory_set(p_dm_odm, &(p_dbg_avg->rssi_cck_avg), 0, sizeof(struct phydm_phystatus_avg));
}
...
)
{
	...
	odm_memory_set(p_dm_odm, &(p_dbg_statistic->rssi_cck_sum), 0, sizeof(struct phydm_phystatus_statistic));
}
...
)
{
	...
	if (p_pktinfo->is_packet_match_bssid) {
		phydm_avg_phystatus_index(p_dm_odm, p_phy_info, p_pktinfo);
