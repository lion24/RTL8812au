...
u32 rtw_rates[] = {1000000,  2000000,  5500000,  11000000, 6000000,  9000000,
...
void rtw_request_wps_pbc_event(_adapter *padapter) {
  ...
  if (padapter->pid[0] == 0) { //	0 is the default value and it means the
                               //application monitors the HW PBC doesn't privde
                               //its pid to driver.
    return;
  }
...
void rtw_indicate_wx_assoc_event(_adapter *padapter) {
  ...
  if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) == _TRUE)
    ...
    _rtw_memcpy(wrqu.ap_addr.sa_data, pmlmepriv->cur_network.network.MacAddress,
                ETH_ALEN);
...
static int search_p2p_wfd_ie(_adapter *padapter, struct iw_request_info *info,
...
#endif // CONFIG_WFD
  {
    if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
      ...
      //	The prefix of SSID should be "DIRECT-" and the IE should
      //contains the P2P IE.
      //	If not, the driver should ignore this AP and go to the next AP.

      //	Verifying the SSID
      if (_rtw_memcmp(pnetwork->network.Ssid.Ssid, pwdinfo->p2p_wildcard_ssid,
                      P2P_WILDCARD_SSID_LEN)) {
        u32 p2pielen = 0;

        //	Verifying the P2P IE
        if (rtw_get_p2p_ie_from_scan_queue(
                &pnetwork->network.IEs[0], pnetwork->network.IELength, NULL,
                &p2pielen, pnetwork->network.Reserved[0])) {
          blnGotP2PIE = _TRUE;
        }
      }
  ...
  if (SCAN_RESULT_WFD_TYPE == pwdinfo->wfd_info->scan_result_type) {
    ...
    if (rtw_get_wfd_ie_from_scan_queue(
            ...
            &wfd_ielen, pnetwork->network.Reserved[0])) {
      u8 wfd_devinfo[6] = {0x00};
      uint wfd_devlen = 6;

      if (rtw_get_wfd_attr_content(wfd_ie, wfd_ielen, WFD_ATTR_DEVICE_INFO,
                                   wfd_devinfo, &wfd_devlen)) {
        if (pwdinfo->wfd_info->wfd_device_type == WFD_DEVINFO_PSINK) {
          //	the first two bits will indicate the WFD device type
          if ((wfd_devinfo[1] & 0x03) == WFD_DEVINFO_SOURCE) {
            //	If this device is Miracast PSink device, the scan reuslt should
            //just provide the Miracast source.
            blnGotWFD = _TRUE;
          }
        } else if (pwdinfo->wfd_info->wfd_device_type == WFD_DEVINFO_SOURCE) {
          //	the first two bits will indicate the WFD device type
          if ((wfd_devinfo[1] & 0x03) == WFD_DEVINFO_PSINK) {
            //	If this device is Miracast source device, the scan reuslt should
            //just provide the Miracast PSink.
...
static inline char *iwe_stream_essid_proess(_adapter *padapter,
  ...
  start =
      iwe_stream_add_point(info, start, stop, iwe, pnetwork->network.Ssid.Ssid);
  return start;
}
...
static inline char *iwe_stream_chan_process(_adapter *padapter,
  ...
  if (pnetwork->network.Configuration.DSConfig <
      1 /*|| pnetwork->network.Configuration.DSConfig>14*/)
...
static inline char *
iwe_stream_encryption_process(_adapter *padapter, struct iw_request_info *info,
                              struct wlan_network *pnetwork, char *start,
                              char *stop, struct iw_event *iwe, u16 cap) {
...
static inline char *iwe_stream_protocol_process(_adapter *padapter,
  ...
  // parsing HT_CAP_IE
  p = rtw_get_ie(&pnetwork->network.IEs[ie_offset], _HT_CAPABILITY_IE_,
                 &ht_ielen, pnetwork->network.IELength - ie_offset);
  if (p && ht_ielen > 0)
    ht_cap = _TRUE;

#ifdef CONFIG_80211AC_VHT
  // parsing VHT_CAP_IE
  p = rtw_get_ie(&pnetwork->network.IEs[ie_offset], EID_VHTCapability,
                 &vht_ielen, pnetwork->network.IELength - ie_offset);
  if (p && vht_ielen > 0)
    vht_cap = _TRUE;
#endif
  ...
  } else if ((rtw_is_cckrates_included(
                 (u8 *)&pnetwork->network.SupportedRates)) == _TRUE) {
    if (ht_cap == _TRUE)
...
static inline char *iwe_stream_rate_process(_adapter *padapter,
  ...
  if (p && vht_ielen > 0) {
    ...
    vht_data_rate =
        rtw_vht_mcs_to_data_rate(CHANNEL_WIDTH_80, short_GI, vht_highest_rate);
  }
#endif
  ...
  while (pnetwork->network.SupportedRates[i] != 0) {
    ...
    if (rate > max_rate)
      max_rate = rate;
    p += snprintf(p, MAX_CUSTOM_LEN - (p - custom), "%d%s ", rate >> 1,
                  (rate & 1) ? ".5" : "");
    i++;
  }
#ifdef CONFIG_80211AC_VHT
  if (vht_cap == _TRUE) {
    ...
    {
      max_rate =
          (bw_40MHz) ? ((short_GI) ? 300 : 270) : ((short_GI) ? 144 : 130);
...
static inline char *iwe_stream_wapi_process(_adapter *padapter,
  ...
  if (pnetwork->network.Reserved[0] != 2) // Probe Request
  {
    ...
    out_len_wapi = rtw_get_wapi_ie(
        pnetwork->network.IEs, pnetwork->network.IELength, wapi_ie, &wapi_len);
    RT_TRACE(_module_rtl871x_mlme_c_, _drv_info_,
             ("rtw_wx_get_scan: ssid=%s\n", pnetwork->network.Ssid.Ssid));
    RT_TRACE(_module_rtl871x_mlme_c_, _drv_info_,
             ("rtw_wx_get_scan: wapi_len=%d \n", wapi_len));
...
static inline char *iwe_stream_rssi_process(_adapter *padapter,
  ...
  {
    /* Do signal scale mapping when using percentage as the unit of signal
     * strength, since the scale mapping is skipped in odm */
  ...
  // iwe.u.qual.level , iwe.u.qual.noise, iwe.u.qual.updated);
...
#if 1
static char *translate_scan(_adapter *padapter, struct iw_request_info *info,
                            struct wlan_network *pnetwork, char *start,
                            char *stop) {
  struct iw_event iwe;
  u16 cap = 0;
  ...
  start =
      iwe_stream_mac_addr_proess(padapter, info, pnetwork, start, stop, &iwe);
  start = iwe_stream_essid_proess(padapter, info, pnetwork, start, stop, &iwe);
  start =
      iwe_stream_protocol_process(padapter, info, pnetwork, start, stop, &iwe);
  if (pnetwork->network.Reserved[0] == 2) // Probe Request
  {
    cap = 0;
  } else {
    _rtw_memcpy((u8 *)&cap, rtw_get_capability_from_ie(pnetwork->network.IEs),
                2);
    cap = le16_to_cpu(cap);
  }

  start =
      iwe_stream_mode_process(padapter, info, pnetwork, start, stop, &iwe, cap);
  start = iwe_stream_chan_process(padapter, info, pnetwork, start, stop, &iwe);
  start = iwe_stream_encryption_process(padapter, info, pnetwork, start, stop,
                                        &iwe, cap);
  start = iwe_stream_rate_process(padapter, info, pnetwork, start, stop, &iwe);
  start =
      iwe_stream_wpa_wpa2_process(padapter, info, pnetwork, start, stop, &iwe);
  ...
  start =
      iwe_stream_net_rsv_process(padapter, info, pnetwork, start, stop, &iwe);
...
static char *translate_scan(_adapter *padapter, struct iw_request_info *info,
  ...
  if (pnetwork->network.Reserved[0] == 2) // Probe Request
  {
    p = rtw_get_ie(&pnetwork->network.IEs[0], _HT_CAPABILITY_IE_, &ht_ielen,
                   pnetwork->network.IELength);
  } else {
    p = rtw_get_ie(&pnetwork->network.IEs[12], _HT_CAPABILITY_IE_, &ht_ielen,
  ...
  if (pnetwork->network.Reserved[0] != 2) // Probe Request
  {
    ...
    u8 *p;
    sint out_len = 0;
    out_len = rtw_get_sec_ie(pnetwork->network.IEs, pnetwork->network.IELength,
                             rsn_ie, &rsn_len, wpa_ie, &wpa_len);
    RT_TRACE(_module_rtl871x_mlme_c_, _drv_info_,
             ("rtw_wx_get_scan: ssid=%s\n", pnetwork->network.Ssid.Ssid));
    RT_TRACE(_module_rtl871x_mlme_c_, _drv_info_,
...
static int wpa_set_auth_algs(struct net_device *dev, u32 value) {
  ...
  if ((value & AUTH_ALG_SHARED_KEY) && (value & AUTH_ALG_OPEN_SYSTEM)) {
    DBG_871X("wpa_set_auth_algs, AUTH_ALG_SHARED_KEY and  AUTH_ALG_OPEN_SYSTEM "
             "[value:0x%x]\n",
             value);
...
static int wpa_set_encryption(struct net_device *dev, struct ieee_param *param,
  ...
  if (param_len <
      (u32)((u8 *)param->u.crypt.key - (u8 *)param) + param->u.crypt.key_len) {
    ret = -EINVAL;
    goto exit;
  }
  ...
  if (strcmp(param->u.crypt.alg, "WEP") == 0) {
    ...
    if (param->u.crypt.set_tx) {
      ...
      //"psecuritypriv->dot11PrivacyKeyIndex=keyid", but can rtw_set_key to
      //fw/cam
      ...
      _rtw_memcpy(&(psecuritypriv->dot11DefKey[wep_key_idx].skey[0]),
      ...
      dot11AuthAlgrthm_8021X) // 802_1x
  {
        ...
        _TRUE) // sta mode
    {
      ...
      if (psta == NULL) {
        ...
        {
          _rtw_memcpy(
              psta->dot118021x_UncstKey.skey, param->u.crypt.key,
          ...
          if (strcmp(param->u.crypt.alg, "TKIP") == 0) // set mic key
          {
            ...
            _rtw_memcpy(psta->dot11tkiptxmickey.skey, &(param->u.crypt.key[16]),
                        8);
            _rtw_memcpy(psta->dot11tkiprxmickey.skey, &(param->u.crypt.key[24]),
                        8);
        ...
        } else // group key
        {
          if (strcmp(param->u.crypt.alg, "TKIP") == 0 ||
            ...
            _rtw_memcpy(
                padapter->securitypriv.dot118021XGrpKey[param->u.crypt.idx]
                    .skey,
                param->u.crypt.key,
                (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
            // only TKIP group key need to install this
            if (param->u.crypt.key_len > 16) {
              _rtw_memcpy(padapter->securitypriv
                              .dot118021XGrptxmickey[param->u.crypt.idx]
                              .skey,
                          &(param->u.crypt.key[16]), 8);
              _rtw_memcpy(padapter->securitypriv
                              .dot118021XGrprxmickey[param->u.crypt.idx]
                              .skey,
            ...
            _rtw_memcpy(
                padapter->securitypriv.dot11wBIPKey[param->u.crypt.idx].skey,
                param->u.crypt.key,
                (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
            /*printk("IGTK key below:\n");
            for(no=0;no<16;no++)
                    printk(" %02x ",
            padapter->securitypriv.dot11wBIPKey[param->u.crypt.idx].skey[no]);
            printk("\n");*/
  ...
  if (strcmp(param->u.crypt.alg, "SMS4") == 0) {
    ...
    u8 WapiASUEPNInitialValueSrc[16] = {0x36, 0x5C, 0x36, 0x5C, 0x36, 0x5C,
                                        0x36, 0x5C, 0x36, 0x5C, 0x36, 0x5C,
                                        0x36, 0x5C, 0x36, 0x5C};
    u8 WapiAEPNInitialValueSrc[16] = {0x37, 0x5C, 0x36, 0x5C, 0x36, 0x5C,
                                      0x36, 0x5C, 0x36, 0x5C, 0x36, 0x5C,
                                      0x36, 0x5C, 0x36, 0x5C};
    u8 WapiAEMultiCastPNInitialValueSrc[16] = {
    ...
    if (param->u.crypt.set_tx == 1) {
      list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
        if (_rtw_memcmp(pWapiSta->PeerMacAddr, param->sta_addr, 6)) {
          ...
          _rtw_memcpy(pWapiSta->lastRxUnicastPNBEQueue, WapiAEPNInitialValueSrc,
                      16);
          _rtw_memcpy(pWapiSta->lastRxUnicastPNBKQueue, WapiAEPNInitialValueSrc,
                      16);
          _rtw_memcpy(pWapiSta->lastRxUnicastPNVIQueue, WapiAEPNInitialValueSrc,
                      16);
          _rtw_memcpy(pWapiSta->lastRxUnicastPNVOQueue, WapiAEPNInitialValueSrc,
                      16);
      ...
      list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
        if (_rtw_memcmp(pWapiSta->PeerMacAddr, get_bssid(pmlmepriv), 6)) {
          ...
          _rtw_memcpy(pWapiSta->lastRxMulticastPN,
                      WapiAEMultiCastPNInitialValueSrc, 16);
...
static int rtw_set_wpa_ie(_adapter *padapter, char *pie, unsigned short ielen) {
  ...
  if (ielen) {
    ...
    if (rtw_parse_wpa_ie(buf, ielen, &group_cipher, &pairwise_cipher, NULL) ==
        _SUCCESS) {
    ...
    if (rtw_parse_wpa2_ie(buf, ielen, &group_cipher, &pairwise_cipher, NULL) ==
        _SUCCESS) {
      padapter->securitypriv.dot11AuthAlgrthm = dot11AuthAlgrthm_8021X;
    ...
    { // set wps_ie
      ...
      while (cnt < ielen) {
        ...
        if ((eid == _VENDOR_SPECIFIC_IE_) &&
            (_rtw_memcmp(&buf[cnt + 2], wps_oui, 4) == _TRUE)) {
          DBG_871X("SET WPS_IE\n");

          padapter->securitypriv.wps_ie_len =
              ((buf[cnt + 1] + 2) < MAX_WPS_IE_LEN) ? (buf[cnt + 1] + 2)
                                                    : MAX_WPS_IE_LEN;

          _rtw_memcpy(padapter->securitypriv.wps_ie, &buf[cnt],
                      padapter->securitypriv.wps_ie_len);
...
static int rtw_wx_get_name(struct net_device *dev, struct iw_request_info *info,
  ...
  if (check_fwstate(pmlmepriv, _FW_LINKED | WIFI_ADHOC_MASTER_STATE) == _TRUE) {
    // parsing HT_CAP_IE
    p = rtw_get_ie(&pcur_bss->IEs[12], _HT_CAPABILITY_IE_, &ht_ielen,
...
static int rtw_wx_set_freq(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
  _func_enter_;
...
static int rtw_wx_get_freq(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
  WLAN_BSSID_EX *pcur_bss = &pmlmepriv->cur_network.network;
  ...
  if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE) {
    // wrqu->freq.m =
    // ieee80211_wlan_frequencies[pcur_bss->Configuration.DSConfig-1] * 100000;
...
static int rtw_wx_set_mode(struct net_device *dev, struct iw_request_info *a,
  ...
  default:
    ...
    ;
    RT_TRACE(
        _module_rtl871x_ioctl_os_c, _drv_err_,
...
static int rtw_wx_set_pmkid(struct net_device *dev, struct iw_request_info *a,
                            union iwreq_data *wrqu, char *extra) {
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  ...
  if (pPMK->cmd == IW_PMKSA_ADD) {
    ...
    for (j = 0; j < NUM_PMKID_CACHE; j++) {
      if (_rtw_memcmp(psecuritypriv->PMKIDList[j].Bssid, strIssueBssid,
                      ETH_ALEN) ==
          _TRUE) { // BSSID is matched, the same AP => rewrite with new PMKID.

        DBG_871X("[rtw_wx_set_pmkid] BSSID exists in the PMKList.\n");

        _rtw_memcpy(psecuritypriv->PMKIDList[j].PMKID, pPMK->pmkid,
                    IW_PMKID_LEN);
    ...
    if (!blInserted) {
      // Find a new entry
      DBG_871X(
          "[rtw_wx_set_pmkid] Use the new entry index = %d for this PMKID.\n",
          psecuritypriv->PMKIDIndex);

      _rtw_memcpy(psecuritypriv->PMKIDList[psecuritypriv->PMKIDIndex].Bssid,
                  strIssueBssid, ETH_ALEN);
      _rtw_memcpy(psecuritypriv->PMKIDList[psecuritypriv->PMKIDIndex].PMKID,
    ...
    for (j = 0; j < NUM_PMKID_CACHE; j++) {
      if (_rtw_memcmp(psecuritypriv->PMKIDList[j].Bssid, strIssueBssid,
                      ETH_ALEN) == _TRUE) { // BSSID is matched, the same AP =>
                                            // Remove this PMKID information and
                                            // reset it.
    ...
    _rtw_memset(&psecuritypriv->PMKIDList[0], 0x00,
...
static int rtw_wx_get_sens(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
#ifdef CONFIG_PLATFORM_ROCKCHIPS
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
...
static int rtw_wx_get_range(struct net_device *dev,
  ...
  for (i = 0, val = 0; i < MAX_CHANNEL_NUM; i++) {
    ...
    if (pmlmeext->channel_set[i].ChannelNum != 0) {
      ...
      range->freq[val].m =
          rtw_ch2freq(pmlmeext->channel_set[i].ChannelNum) * 100000;
      range->freq[val].e = 1;
      val++;
    }
...
static int rtw_wx_set_wap(struct net_device *dev, struct iw_request_info *info,
...
*/
  ...
  // set_802_11_encryption_mode(padapter,
  // padapter->securitypriv.ndisencryptstatus);
...
static int rtw_wx_get_wap(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra) {
...
static int rtw_wx_set_mlme(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
#if 0
/* SIOCSIWMLME data */
struct	iw_mlme
  ...
  if (wrqu->data.length == sizeof(struct iw_scan_req)) {
    ...
    if (wrqu->data.flags & IW_SCAN_THIS_ESSID) {
      ...
      if (wrqu->data.length >= WEXT_CSCAN_HEADER_SIZE &&
          _rtw_memcmp(extra, WEXT_CSCAN_HEADER, WEXT_CSCAN_HEADER_SIZE) ==
              _TRUE) {
    ...
    _status =
        rtw_set_802_11_bssid_list_scan(padapter, ssid, RTW_SSID_SCAN_AMOUNT);
  ...
  */
  ...
  if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
    ...
    if (padapter->chip_type == RTL8192D)
      wait_for_surveydone =
          300; //	Because the 8192du supports more channels.
    else
  ...
  while (1) {
    ...
    // report network only if the current channel set contains the channel to
    // which this network belongs
    if (rtw_ch_set_search_ch(padapter->mlmeextpriv.channel_set,
                             pnetwork->network.Configuration.DSConfig) >= 0 &&
        rtw_mlme_band_check(
            padapter, pnetwork->network.Configuration.DSConfig) == _TRUE &&
...
static int rtw_wx_set_essid(struct net_device *dev, struct iw_request_info *a,
                            union iwreq_data *wrqu, char *extra) {
  _irqL irqL;
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
...
//		wrq.u.essid.length--;
//	=========================================
//	That means, if the WIRELESS_EXT less than or equal to 20, the correct
//ssid len should subtract 1.
#if WIRELESS_EXT <= 20
    ...
    while (1) {
      if (rtw_end_of_queue_search(phead, pmlmepriv->pscanned) == _TRUE) {
        ...
        RT_TRACE(_module_rtl871x_ioctl_os_c, _drv_warning_,
                 ("rtw_wx_set_essid: scan_q is empty, set ssid to check if "
                  "scanning again!\n"));
      ...
      RT_TRACE(
          _module_rtl871x_ioctl_os_c, _drv_info_,
      ...
      if ((_rtw_memcmp(dst_ssid, src_ssid, ndis_ssid.SsidLength) == _TRUE) &&
        ...
        if (rtw_set_802_11_infrastructure_mode(
                padapter, pnetwork->network.InfrastructureMode) == _FALSE) {
          ret = -1;
          _exit_critical_bh(&queue->lock, &irqL);
          goto exit;
        }
...
static int rtw_wx_get_essid(struct net_device *dev, struct iw_request_info *a,
                            union iwreq_data *wrqu, char *extra) {
  u32 len, ret = 0;
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
...
static int rtw_wx_set_rate(struct net_device *dev, struct iw_request_info *a,
                           union iwreq_data *wrqu, char *extra) {
  int i, ret = 0;
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
...
static int rtw_wx_get_rate(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
  u16 max_rate = 0;
...
static int rtw_wx_set_rts(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra) {
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  ...
  if (wrqu->rts.disabled)
    ...
    if (wrqu->rts.value < 0 || wrqu->rts.value > 2347)
      return -EINVAL;
...
static int rtw_wx_get_rts(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra) {
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
...
static int rtw_wx_set_frag(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
...
static int rtw_wx_get_frag(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
...
static int rtw_wx_set_enc(struct net_device *dev, struct iw_request_info *info,
  ...
  if (erq->flags & IW_ENCODE_OPEN) {
    ...
    padapter->securitypriv.ndisencryptstatus =
        Ndis802_11Encryption1Enabled; // Ndis802_11EncryptionDisabled;
  ...
  if (erq->length > 0) {
    ...
    if (keyindex_provided ==
        1) // set key_id only, no given KeyMaterial(erq->length==0).
    {
      padapter->securitypriv.dot11PrivacyKeyIndex = key;

      DBG_871X("(keyindex_provided == 1), keyid=%d, key_len=%d\n", key,
               padapter->securitypriv.dot11DefKeylen[key]);
...
static int rtw_wx_get_enc(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *keybuf) {
  uint key, ret = 0;
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  struct iw_point *erq = &(wrqu->encoding);
  ...
  case Ndis802_11Encryption1Enabled:
    ...
    if (erq->length) {
      _rtw_memcpy(keybuf, padapter->securitypriv.dot11DefKey[key].skey,
                  padapter->securitypriv.dot11DefKeylen[key]);
...
static int rtw_wx_set_auth(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  ...
  case IW_AUTH_TKIP_COUNTERMEASURES: {
    if (param->value) { // wpa_supplicant is enabling the tkip countermeasure.
      padapter->securitypriv.btkip_countermeasure = _TRUE;
    } else { // wpa_supplicant is disabling the tkip countermeasure.
  ...
  case IW_AUTH_DROP_UNENCRYPTED: {
    ...
    if (padapter->securitypriv.ndisencryptstatus ==
      ...
      break; // it means init value, or using wep, ndisencryptstatus =
             // Ndis802_11Encryption1Enabled,
...
static int rtw_wx_get_nick(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
  //_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  // struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
  // struct security_priv *psecuritypriv = &padapter->securitypriv;
...
/*
   ...
   shared, and 8021x
        u32 dot11PrivacyAlgrthm;	// This specify the privacy for shared
   auth. algorithm.
...
static int rtw_wx_read32(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra) {
  PADAPTER padapter;
  struct iw_point *p;
  u16 len;
...
static int rtw_wx_write32(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra) {
...
static int rtw_wx_read_rf(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra) {
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
...
static int rtw_wx_write_rf(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  u32 path, addr, data32;
...
struct	iw_point
{
  ...
  RT_TRACE(
      _module_rtl871x_ioctl_os_c, _drv_info_,
      ("drvext set oid subcode [%d], len[%d], InformationBufferLength[%d]\r\n",
       poidparam->subcode, poidparam->len, len));

  // check subcode
  if (poidparam->subcode >= MAX_DRVEXT_HANDLERS) {
    RT_TRACE(_module_rtl871x_ioctl_os_c, _drv_err_,
             ("no matching drvext handlers\r\n"));
    ret = -EINVAL;
    goto _rtw_drvext_hdl_exit;
  }
...
static int rtw_mp_ioctl_hdl(struct net_device *dev,
                            struct iw_request_info *info,
                            union iwreq_data *wrqu, char *extra) {
  int ret = 0;
...
#endif
  {
    rtw_dbg_mode_hdl(padapter, poidparam->subcode, poidparam->data,
                     poidparam->len);
  }
...
static int rtw_get_ap_info(struct net_device *dev, struct iw_request_info *info,
  ...
  while (1) {
    ...
    if (_rtw_memcmp(bssid, pnetwork->network.MacAddress, ETH_ALEN) ==
        _TRUE) // BSSID match, then check if supporting wpa/wpa2
    {
...
static int rtw_set_pid(struct net_device *dev, struct iw_request_info *info,
                       union iwreq_data *wrqu, char *extra) {
...
static int rtw_wps_start(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra) {
  ...
  if ((_TRUE == padapter->bDriverStopped) ||
      (_TRUE == padapter->bSurpriseRemoved) || (NULL == pdata)) {
    ret = -EINVAL;
    goto exit;
  }
...
static int rtw_p2p_set_op_ch(struct net_device *dev,
  ...
  //	This function is used to set the operating channel if the driver will
...
static int rtw_p2p_profilefound(struct net_device *dev,
  ...
  //	The upper application should pass the SSID to driver by using this
  //rtw_p2p_profilefound function.
  if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
    if (extra[0] == '0') {
      //	Remove all the profile information of wifidirect_info structure.
      _rtw_memset(&pwdinfo->profileinfo[0], 0x00,
                  sizeof(struct profile_info) * P2P_MAX_PERSISTENT_GROUP_NUM);
      ...
      if (pwdinfo->profileindex >= P2P_MAX_PERSISTENT_GROUP_NUM) {
        ...
        // pwdinfo->profileinfo[ pwdinfo->profileindex ].ssidlen = ( extra[18] -
        // '0' ) * 10 + ( extra[ 19 ] - '0' );
        //_rtw_memcpy( pwdinfo->profileinfo[ pwdinfo->profileindex ].ssid,
        //&extra[ 20 ], pwdinfo->profileinfo[ pwdinfo->profileindex ].ssidlen );
...
static int rtw_p2p_setDN(struct net_device *dev, struct iw_request_info *info,
                         union iwreq_data *wrqu, char *extra) {
...
static int rtw_p2p_get_status(struct net_device *dev,
  ...
  //information.
  //	About the "Role" information, we will use the new private IOCTL to get
...

{
  ...
  DBG_871X("[%s] Role = %d, Status = %d, peer addr = "
           "%.2X:%.2X:%.2X:%.2X:%.2X:%.2X\n",
           __FUNCTION__, rtw_p2p_role(pwdinfo), rtw_p2p_state(pwdinfo),
           pwdinfo->rx_prov_disc_info.peerDevAddr[0],
           pwdinfo->rx_prov_disc_info.peerDevAddr[1],
           pwdinfo->rx_prov_disc_info.peerDevAddr[2],
           pwdinfo->rx_prov_disc_info.peerDevAddr[3],
           pwdinfo->rx_prov_disc_info.peerDevAddr[4],
           pwdinfo->rx_prov_disc_info.peerDevAddr[5]);
  sprintf(extra, "\n%.2X%.2X%.2X%.2X%.2X%.2X",
          pwdinfo->rx_prov_disc_info.peerDevAddr[0],
          pwdinfo->rx_prov_disc_info.peerDevAddr[1],
          pwdinfo->rx_prov_disc_info.peerDevAddr[2],
...

{
  ...
  sprintf(extra, "\n%.2X:%.2X:%.2X:%.2X:%.2X:%.2X %s",
          pwdinfo->groupid_info.go_device_addr[0],
          pwdinfo->groupid_info.go_device_addr[1],
          pwdinfo->groupid_info.go_device_addr[2],
          pwdinfo->groupid_info.go_device_addr[3],
          pwdinfo->groupid_info.go_device_addr[4],
          pwdinfo->groupid_info.go_device_addr[5], pwdinfo->groupid_info.ssid);
  wrqu->data.length = strlen(extra);
  return ret;
}
...
static int rtw_p2p_get_wps_configmethod(struct net_device *dev,
  ...
  //	The input data is the MAC address which the application wants to know
  //its WPS config method.
  //	After knowing its WPS config method, the application can decide the
  //config method for provisioning discovery.
  ...
  while (1) {
    ...
    if (_rtw_memcmp(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
      ...
      if ((wpsie = rtw_get_wps_ie_from_scan_queue(
               &pnetwork->network.IEs[0], pnetwork->network.IELength, NULL,
               &wpsie_len, pnetwork->network.Reserved[0]))) {
        rtw_get_wps_attr_content(wpsie, wpsie_len, WPS_ATTR_CONF_METHOD,
                                 (u8 *)&attr_content, &attr_contentlen);
        if (attr_contentlen) {
...
static int
rtw_p2p_get_peer_wfd_preferred_connection(struct net_device *dev,
                                          struct iw_request_info *info,
                                          union iwreq_data *wrqu, char *extra) {
  ...
  wrqu->data.length = strlen(extra);
  pwdinfo->wfd_info->wfd_pc =
      _FALSE; //	Reset the WFD preferred connection to P2P
  return ret;
}
...
static int rtw_p2p_get_peer_wfd_session_available(struct net_device *dev,
  ...
  wrqu->data.length = strlen(extra);
  pwdinfo->wfd_info->peer_session_avail =
      _TRUE; //	Reset the WFD session available
  return ret;
}
...
static int rtw_p2p_get_go_device_address(struct net_device *dev,
  ...
  //	The input data is the GO's interface address which the application wants
  //to know its device address.
  ...
  while (1) {
    ...
    if (_rtw_memcmp(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
      ...
      //	This is for the case that the P2P device address is not the same
      //as the P2P interface address.

      if ((p2pie = rtw_get_p2p_ie_from_scan_queue(
               &pnetwork->network.IEs[0], pnetwork->network.IELength, NULL,
               &p2pielen, pnetwork->network.Reserved[0]))) {
        while (p2pie) {
          //	The P2P Device ID attribute is included in the Beacon frame.
          //	The P2P Device Info attribute is included in the probe response
          //frame.

          _rtw_memset(attr_content, 0x00, 100);
          if (rtw_get_p2p_attr_content(p2pie, p2pielen, P2P_ATTR_DEVICE_ID,
                                       attr_content, &attr_contentlen)) {
            //	Handle the P2P Device ID attribute of Beacon first
            blnMatch = 1;
            break;

          } else if (rtw_get_p2p_attr_content(p2pie, p2pielen,
          ...
          // Get the next P2P IE
          p2pie = rtw_get_p2p_ie(
              p2pie + p2pielen,
              pnetwork->network.IELength - 12 -
                  (p2pie - &pnetwork->network.IEs[12] + p2pielen),
              NULL, &p2pielen);
        }
      }
    }
  ...
  if (!blnMatch) {
    ...
    sprintf(go_devadd_str, "\n\ndev_add=%.2X:%.2X:%.2X:%.2X:%.2X:%.2X",
            attr_content[0], attr_content[1], attr_content[2], attr_content[3],
            attr_content[4], attr_content[5]);
  }
...
static int rtw_p2p_get_device_type(struct net_device *dev,
  ...
  u8 dev_type[8] = {0x00};
  uint dev_type_len = 0;
  u8 dev_type_str[P2P_PRIVATE_IOCTL_SET_LEN] = {0x00}; // +9 is for the str
                                                       // "dev_type=", we have
                                                       // to clear it at
                                                       // wrqu->data.pointer
  ...
  while (1) {
    ...
    if (_rtw_memcmp(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
      ...
      if ((wpsie = rtw_get_wps_ie_from_scan_queue(
               &pnetwork->network.IEs[0], pnetwork->network.IELength, NULL,
               &wpsie_len, pnetwork->network.Reserved[0]))) {
        rtw_get_wps_attr_content(wpsie, wpsie_len, WPS_ATTR_PRIMARY_DEV_TYPE,
                                 dev_type, &dev_type_len);
        if (dev_type_len) {
          u16 type = 0;
...
static int rtw_p2p_get_device_name(struct net_device *dev,
  ...
  //	The input data is the MAC address which the application wants to know
  //its device name.
  //	Such user interface could show peer device's device name instead of
  //ssid.
  ...
  while (1) {
    ...
    if (_rtw_memcmp(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
      ...
      if ((wpsie = rtw_get_wps_ie_from_scan_queue(
               &pnetwork->network.IEs[0], pnetwork->network.IELength, NULL,
               &wpsie_len, pnetwork->network.Reserved[0]))) {
        rtw_get_wps_attr_content(wpsie, wpsie_len, WPS_ATTR_DEVICE_NAME,
                                 dev_name, &dev_len);
        if (dev_len) {
    ...
    if (_rtw_memcmp(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
      ...
      if ((p2pie = rtw_get_p2p_ie_from_scan_queue(
        ...
        while (p2pie) {
          ...
          if (rtw_get_p2p_attr_content(p2pie, p2pielen, P2P_ATTR_CAPABILITY,
                                       attr_content, &attr_contentlen)) {
...
static int rtw_p2p_connect(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
  ...
  //	2. Second information is the WPS PINCode or "pbc" string for push button
  //method
  ...
  if (uintPeerChannel) {
    ...
    _rtw_memcpy(pwdinfo->nego_req_info.peerDevAddr,
                pnetwork->network.MacAddress, ETH_ALEN);
    pwdinfo->nego_req_info.benable = _TRUE;

    _cancel_timer_ex(&pwdinfo->restore_p2p_state_timer);
    if (rtw_p2p_state(pwdinfo) != P2P_STATE_GONEGO_OK) {
      //	Restore to the listen state if the current p2p state is not nego
      //OK
...
static int rtw_p2p_invite_req(struct net_device *dev,
  ...
  //	The input data contains two informations.
  //	1. First information is the P2P device address which you want to send
  //to.
  //	2. Second information is the group id which combines with GO's mac
  //address, space and GO's ssid.
  //	Command line sample: iwpriv wlan0 p2p_set invite="00:11:22:33:44:55
  //00:E0:4C:00:00:05 DIRECT-xy"
  ...
  for (jj = 0, kk = 0; jj < ETH_ALEN; jj++, kk += 3) {
    pinvite_req_info->peer_macaddr[jj] =
        key_2char2num(extra[kk], extra[kk + 1]);
  }
  ...
  while (1) {
    ...
    //	Commented by Albert 2011/05/18
    //	Match the device address located in the P2P IE
    //	This is for the case that the P2P device address is not the same as the
    ...
    if ((p2pie = rtw_get_p2p_ie_from_scan_queue(
      ...
      if (rtw_get_p2p_attr_content(p2pie, p2pielen, P2P_ATTR_DEVICE_ID,
                                   attr_content, &attr_contentlen)) {
        //	Handle the P2P Device ID attribute of Beacon first
        if (_rtw_memcmp(attr_content, pinvite_req_info->peer_macaddr,
                        ETH_ALEN)) {
          uintPeerChannel = pnetwork->network.Configuration.DSConfig;
          break;
        }
      } else if (rtw_get_p2p_attr_content(p2pie, p2pielen, P2P_ATTR_DEVICE_INFO,
                                          attr_content, &attr_contentlen)) {
        //	Handle the P2P Device Info attribute of probe response
        if (_rtw_memcmp(attr_content, pinvite_req_info->peer_macaddr,
                        ETH_ALEN)) {
  ...
  if (uintPeerChannel) {
    ...
    if (rtw_get_wfd_ie_from_scan_queue(
      ...
      if (rtw_get_wfd_attr_content(wfd_ie, wfd_ielen, WFD_ATTR_DEVICE_INFO,
        ...
        //	The first two bytes are the WFD device information field of WFD
  ...
  if (uintPeerChannel) {
    ...
    _rtw_memcpy(pinvite_req_info->go_ssid, &extra[36],
                (u32)pinvite_req_info->ssidlen);
    ...
    if (check_fwstate(pbuddy_mlmepriv, _FW_LINKED)) {
      ...
      set_channel_bwmode(padapter, uintPeerChannel,
                         HAL_PRIME_CHNL_OFFSET_DONT_CARE, CHANNEL_WIDTH_20);
    }
#else
    set_channel_bwmode(padapter, uintPeerChannel,
...
#ifdef CONFIG_WFD
static int rtw_p2p_set_pc(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra) {
  ...
  //	1. Input information is the MAC address which wants to know the
  //Preferred Connection bit (PC bit)
  //	Format: 00:E0:4C:00:00:05
...
static int rtw_p2p_set_sa(struct net_device *dev, struct iw_request_info *info,
                          union iwreq_data *wrqu, char *extra) {
...
static int rtw_p2p_prov_disc(struct net_device *dev,
  ...
  //	The input data contains two informations.
  //	1. First information is the MAC address which wants to issue the
  //provisioning discovery request frame.
  //	2. Second information is the WPS configuration method which wants to
  //discovery
  ...
  if (pwdinfo->p2p_state == P2P_STATE_NONE) {
    ...
    //	Reset the content of struct tx_provdisc_req_info excluded the
    //wps_config_method_request.
    _rtw_memset(pwdinfo->tx_prov_disc_info.peerDevAddr, 0x00, ETH_ALEN);
    _rtw_memset(pwdinfo->tx_prov_disc_info.peerIFAddr, 0x00, ETH_ALEN);
    _rtw_memset(&pwdinfo->tx_prov_disc_info.ssid, 0x00,
  ...
  while (1) {
    ...
    if ((p2pie = rtw_get_p2p_ie_from_scan_queue(
      ...
      while (p2pie) {
        ...
        if (rtw_get_p2p_attr_content(p2pie, p2pielen, P2P_ATTR_DEVICE_ID,
          ...
          if (_rtw_memcmp(attr_content, peerMAC, ETH_ALEN)) {
            ...
            break;
                                            ...
                                            P2P_ATTR_DEVICE_INFO, attr_content,
                                            &attr_contentlen)) {
          //	Handle the P2P Device Info attribute of probe response
          if (_rtw_memcmp(attr_content, peerMAC, ETH_ALEN)) {
            uintPeerChannel = pnetwork->network.Configuration.DSConfig;
        ...
        p2pie = rtw_get_p2p_ie(
            p2pie + p2pielen,
            pnetwork->network.IELength - ie_offset -
                (p2pie - &pnetwork->network.IEs[ie_offset] + p2pielen),
            NULL, &p2pielen);
      }
    }
  ...
  if (uintPeerChannel) {
...
#endif // CONFIG_CONCURRENT_MODE
    _rtw_memcpy(pwdinfo->tx_prov_disc_info.peerIFAddr,
                pnetwork->network.MacAddress, ETH_ALEN);
    ...
    if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_CLIENT)) {
      _rtw_memcpy(&pwdinfo->tx_prov_disc_info.ssid, &pnetwork->network.Ssid,
                  sizeof(NDIS_802_11_SSID));
    } else if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_DEVICE) ||
               rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)) {
      _rtw_memcpy(pwdinfo->tx_prov_disc_info.ssid.Ssid,
...
//	This function is used to inform the driver the user had specified the
//pin code value or pbc
//	to application.
...
static int rtw_p2p_got_wpsinfo(struct net_device *dev,
  ...
  //	if the input data is P2P_GOT_WPSINFO_PEER_DISPLAY_PIN -> the utility
  //just input the PIN code got from the peer P2P device.
  //	if the input data is P2P_GOT_WPSINFO_SELF_DISPLAY_PIN -> the utility
  //just got the PIN code from itself.
  //	if the input data is P2P_GOT_WPSINFO_PBC -> the utility just determine
  //to use the PBC
...
static int rtw_p2p_set(struct net_device *dev, struct iw_request_info *info,
                       union iwreq_data *wrqu, char *extra) {
...
static int rtw_p2p_get(struct net_device *dev, struct iw_request_info *info,
                       union iwreq_data *wrqu, char *extra) {
  ...
  if (_rtw_memcmp(wrqu->data.pointer, "status", 6)) {
    ...
    //	Get the P2P device address when receiving the provision discovery
    //request frame.
...
static int rtw_p2p_get2(struct net_device *dev, struct iw_request_info *info,
                        union iwreq_data *wrqu, char *extra) {
...
u8 dump_cmd_id = 0;
#endif
static int rtw_dbg_port(struct net_device *dev, struct iw_request_info *info,
                        union iwreq_data *wrqu, char *extra) {
  _irqL irqL;
  int ret = 0;
  ...
  case 0x72: // read_bb
    DBG_871X("read_bbreg(0x%x)=0x%x\n", arg,
             rtw_hal_read_bbreg(padapter, arg, 0xffffffff));
    break;
  case 0x73: // write_bb
  ...
  case 0x75: // write_rf
    ...
    DBG_871X("write RF_reg path(0x%02x),offset(0x%x),value(0x%08x)\n",
             minor_cmd, arg,
             rtw_hal_read_rfreg(padapter, minor_cmd, arg, 0xffffffff));
    break;
  ...
  case 0x78: // IOL test
    ...
    case 0x05: // blink LED test
    {
...

      {
        ...
        if (_SUCCESS !=
            rtw_IOL_exec_cmds_sync(padapter, xmit_frame,
                                   (blink_delay_ms * blink_num * 2) + 200, 0))
          ret = -EPERM;
      }
    } break;
    ...
    case 0x06: // continuous wirte byte test
    {
      ...
      if (start_value + write_num - 1 == (final = rtw_read8(padapter, reg))) {
        DBG_871X("continuous IOL_CMD_WB_REG to 0x%x %u times Success, "
                 "start:%u, final:%u\n",
                 reg, write_num, start_value, final);
      } else {
        DBG_871X("continuous IOL_CMD_WB_REG to 0x%x %u times Fail, start:%u, "
                 "final:%u\n",
                 reg, write_num, start_value, final);
      }
    } break;
    ...
    case 0x07: // continuous wirte word test
    {
      ...
      if (start_value + write_num - 1 == (final = rtw_read16(padapter, reg))) {
        DBG_871X("continuous IOL_CMD_WW_REG to 0x%x %u times Success, "
                 "start:%u, final:%u\n",
                 reg, write_num, start_value, final);
      } else {
        DBG_871X("continuous IOL_CMD_WW_REG to 0x%x %u times Fail, start:%u, "
                 "final:%u\n",
                 reg, write_num, start_value, final);
      }
    } break;
    ...
    case 0x08: // continuous wirte dword test
    {
      ...
      if (start_value + write_num - 1 == (final = rtw_read32(padapter, reg))) {
        DBG_871X("continuous IOL_CMD_WD_REG to 0x%x %u times Success, "
                 "start:%u, final:%u\n",
                 reg, write_num, start_value, final);
      } else {
        DBG_871X("continuous IOL_CMD_WD_REG to 0x%x %u times Fail, start:%u, "
                 "final:%u\n",
                 reg, write_num, start_value, final);
      }
    } break;
  ...
  case 0x7F:
    ...
    case 0x01:
      DBG_871X("auth_alg=0x%x, enc_alg=0x%x, auth_type=0x%x, enc_type=0x%x\n",
               psecuritypriv->dot11AuthAlgrthm,
               psecuritypriv->dot11PrivacyAlgrthm, psecuritypriv->ndisauthtype,
               psecuritypriv->ndisencryptstatus);
      break;
    case 0x02:
    ...
    case 0x05:
      ...
      if (psta) {
        ...
        DBG_871X("state=0x%x, aid=%d, macid=%d, raid=%d\n", psta->state,
                 psta->aid, psta->mac_id, psta->raid);
#ifdef CONFIG_80211N_HT
        DBG_871X("qos_en=%d, ht_en=%d, init_rate=%d\n", psta->qos_option,
                 psta->htpriv.ht_option, psta->init_rate);
    ...
    {
      // u8 driver_vcs_en; //Enable=1, Disable=0 driver control
      // vrtl_carrier_sense.
    ...
    case 0x11: // dump linked status
    {
      ...
      if (extra_arg == 1 ||
          (extra_arg == 0 && pre_mode == 1)) // not consider pwr_saving 0:
      {
        padapter->bLinkInfoDump = extra_arg;

      } else if ((extra_arg == 2) ||
                 (extra_arg == 0 && pre_mode == 2)) // consider power_saving
      {
    ...
    {
      struct registry_priv *pregpriv = &padapter->registrypriv;
      // 0: disable, bit(0):enable 2.4g, bit(1):enable 5g, 0x3: enable both 2.4g
      // and 5g
      // default is set to enable 2.4GHZ for IOT issue with bufflao's AP at 5GHZ
      if (pregpriv && (extra_arg == 0 || extra_arg == 1 || extra_arg == 2 ||
                       extra_arg == 3)) {
    ...
    {
      struct registry_priv *pregpriv = &padapter->registrypriv;
      // 0: disable, 0x1:enable (but wifi_spec should be 0), 0x2: force enable
      // (don't care wifi_spec)
    ...
    case 0x1b: {
      ...
      if (arg == 0) {
        DBG_871X(
            "disable driver ctrl max_rx_rate, reset to default_rate_set\n");
...
static int wpa_supplicant_ioctl(struct net_device *dev, struct iw_point *p) {
  ...
  case IEEE_CMD_SET_WPA_IE:
    ...
    ret =
        rtw_set_wpa_ie((_adapter *)rtw_netdev_priv(dev),
...
#ifdef CONFIG_AP_MODE
static int rtw_set_encryption(struct net_device *dev, struct ieee_param *param,
                              u32 param_len) {
  int ret = 0;
  u32 wep_key_idx, wep_key_len, wep_total_len;
  NDIS_802_11_WEP *pwep = NULL;
  ...
  if (param->sta_addr[0] == 0xff && param->sta_addr[1] == 0xff &&
    ...
    if (!psta) {
      // ret = -EINVAL;
      DBG_871X("rtw_set_encryption(), sta has already been removed or never "
               "been added\n");
      goto exit;
    }
  }
  ...
  if (strcmp(param->u.crypt.alg, "WEP") == 0 && (psta == NULL)) {
    ...
    if (param->u.crypt.set_tx) {
      ...
      rtw_ap_set_wep_key(padapter, pwep->KeyMaterial, pwep->KeyLength,
                         wep_key_idx, 1);
    } else {
      DBG_871X("wep, set_tx=0\n");
  ...
  if (!psta && check_fwstate(pmlmepriv, WIFI_AP_STATE)) // //group key
  {
    if (param->u.crypt.set_tx == 1) {
      if (strcmp(param->u.crypt.alg, "WEP") == 0) {
        ...
        _rtw_memcpy(
            psecuritypriv->dot118021XGrpKey[param->u.crypt.idx].skey,
            param->u.crypt.key,
            (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
        ...
        // set mic key
        _rtw_memcpy(
            psecuritypriv->dot118021XGrptxmickey[param->u.crypt.idx].skey,
            &(param->u.crypt.key[16]), 8);
        _rtw_memcpy(
            psecuritypriv->dot118021XGrprxmickey[param->u.crypt.idx].skey,
            &(param->u.crypt.key[24]), 8);
      ...
      if (pbcmc_sta) {
        pbcmc_sta->ieee8021x_blocked = _FALSE;
        pbcmc_sta->dot118021XPrivacy =
            psecuritypriv->dot118021XGrpPrivacy; // rx will use bmc_sta's
                                                 // dot118021XPrivacy
      }
    }
...
static int rtw_set_beacon(struct net_device *dev, struct ieee_param *param,
                          int len) {
  int ret = 0;
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
  ...
  if (rtw_check_beacon_data(padapter, pbuf, (len - 12 - 2)) ==
      _SUCCESS) // 12 = param header, 2:no packed
    ret = 0;
  else
    ret = -EINVAL;
  ...
  */
  ...
  if (psta) {
    ...
    if (WLAN_STA_HT & flags) {
      ...
      psta->qos_option = 1;
      _rtw_memcpy((void *)&psta->htpriv.ht_cap,
                  (void *)&param->u.add_sta.ht_cap,
...
static int rtw_ioctl_get_sta_data(struct net_device *dev,
  ...
  if (psta) {
    ...
    */
    ...
    psta_data->sta_set =
        ((psta->nonerp_set) | (psta->no_short_slot_time_set << 1) |
         (psta->no_short_preamble_set << 2) | (psta->no_ht_gf_set << 3) |
         (psta->no_ht_set << 4) | (psta->ht_20mhz_set << 5));

    psta_data->tx_supp_rates_len = psta->bssratelen;
    _rtw_memcpy(psta_data->tx_supp_rates, psta->bssrateset, psta->bssratelen);
#ifdef CONFIG_80211N_HT
    _rtw_memcpy(&psta_data->ht_cap, &psta->htpriv.ht_cap,
                sizeof(struct rtw_ieee80211_ht_cap));
#endif // CONFIG_80211N_HT
...
static int rtw_get_sta_wpaie(struct net_device *dev, struct ieee_param *param) {
  ...
  if (psta) {
    if ((psta->wpa_ie[0] == WLAN_EID_RSN) ||
        (psta->wpa_ie[0] == WLAN_EID_GENERIC)) {
      int wpa_ie_len;
      int copy_len;
...
static int rtw_set_wps_beacon(struct net_device *dev, struct ieee_param *param,
                              int len) {
  int ret = 0;
...
static int rtw_set_hidden_ssid(struct net_device *dev, struct ieee_param *param,
                               int len) {
  int ret = 0;
  ...
  mlmeinfo->hidden_ssid_mode = ignore_broadcast_ssid =
      param->u.bcn_ie.reserved[1];
  ...
  if (ssid_ie && ssid_len > 0 && ssid_len <= NDIS_802_11_LENGTH_SSID) {
    ...
    if (0)
      DBG_871X(FUNC_ADPT_FMT " ssid:(%s,%d), from ie:(%s,%d), (%s,%d)\n",
               FUNC_ADPT_ARG(adapter), ssid, ssid_len, pbss_network->Ssid.Ssid,
               pbss_network->Ssid.SsidLength, pbss_network_ext->Ssid.Ssid,
               pbss_network_ext->Ssid.SsidLength);
  ...
  DBG_871X(FUNC_ADPT_FMT " ignore_broadcast_ssid:%d, %s,%d\n",
           FUNC_ADPT_ARG(adapter), ignore_broadcast_ssid, ssid, ssid_len);

  return ret;
}

static int rtw_ioctl_acl_remove_sta(struct net_device *dev,
                                    struct ieee_param *param, int len) {
  int ret = 0;
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
...
static int rtw_ioctl_acl_add_sta(struct net_device *dev,
                                 struct ieee_param *param, int len) {
  int ret = 0;
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
...
static int rtw_ioctl_set_macaddr_acl(struct net_device *dev,
                                     struct ieee_param *param, int len) {
  int ret = 0;
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
  struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
...
static int rtw_wx_set_priv(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *awrq, char *extra) {
...
//	  dev->name, ext));
  ...
  if (dwrq->flags == 0x8766 && len > 8) {
    ...
    if ((_VENDOR_SPECIFIC_IE_ == probereq_wpsie[0]) &&
      ...
      cp_sz = probereq_wpsie_len > MAX_WPS_IE_LEN ? MAX_WPS_IE_LEN
                                                  : probereq_wpsie_len;
  ...
  case ANDROID_WIFI_CMD_RSSI: {
    ...
    if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE) {
      sprintf(ext, "%s rssi %d", pcur_network->network.Ssid.Ssid,
  ...
  if (copy_to_user(dwrq->pointer, ext,
                   min(dwrq->length, (u16)(strlen(ext) + 1))))
    ret = -EFAULT;

#ifdef CONFIG_DEBUG_RTW_WX_SET_PRIV
  DBG_871X("%s: %s req=%s rep=%s dwrq->length=%d, strlen(ext)+1=%d\n",
           __FUNCTION__, dev->name, ext_dbg, ext, dwrq->length,
           (u16)(strlen(ext) + 1));
#endif
...
#ifdef CONFIG_WOWLAN
static int rtw_wowlan_ctrl(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
...
static int rtw_pm_set(struct net_device *dev, struct iw_request_info *info,
                      union iwreq_data *wrqu, char *extra) {
  int ret = 0;
  unsigned mode = 0;
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
...
static int rtw_mp_efuse_get(struct net_device *dev,
...
#ifdef CONFIG_IOL
  u8 org_fw_iol =
      padapter->registrypriv.fw_iol; // 0:Disable, 1:enable, 2:by usb speed
#endif
  ...
  if (strcmp(tmp[0], "status") == 0) {
    sprintf(extra, "Load File efuse=%s,Load File MAC=%s",
            (pEEPROM->bloadfile_fail_flag ? "FAIL" : "OK"),
            (pEEPROM->bloadmac_fail_flag ? "FAIL" : "OK"));
    ...
    mapLen = EFUSE_MAP_SIZE;
    if (rtw_efuse_map_read(padapter, EFUSE_WIFI, mapLen,
                           pEfuseHal->fakeEfuseInitMap) == _FAIL) {
    ...
    *extra = 0;
    ...
    if (rtw_BT_efuse_map_read(padapter, 0, mapLen, pEfuseHal->BTEfuseInitMap) ==
        _FAIL) {
    ...
    sprintf(extra, "\n");
    for (i = 0; i < 512;
         i += 16) // set 512 because the iwpriv's extra size have limit 0x7FF
    {
      //			DBG_871X("0x%03x\t", i);
    ...
    *extra = 0;
    ...
    for (i = 0; i < 512; i += 16) {
      ...
      for (j = 0; j < 8; j++) {
        //				DBG_871X("%02X ",
        //pEfuseHal->fakeBTEfuseModifiedMap[i+j]);
    ...
    for (i = 0; i < EFUSE_MAP_SIZE; i += 16) {
      ...
      for (j = 0; j < 8; j++) {
        //				DBG_871X("%02X ",
        //pEfuseHal->fakeEfuseModifiedMap[i+j]);
        sprintf(extra, "%s%02X ", extra,
                pEfuseHal->fakeEfuseModifiedMap[i + j]);
      }
      //			DBG_871X("\t");
...
#ifdef CONFIG_IOL
  padapter->registrypriv.fw_iol =
      org_fw_iol; // 0:Disable, 1:enable, 2:by usb speed
#endif
  return err;
}
...
static int rtw_mp_efuse_set(struct net_device *dev,
  ...
  if (strcmp(tmp[0], "wmap") == 0) {
    ...
    EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI,
                             ...
                             (PVOID)&max_available_size, _FALSE);
#else
    // Change to check TYPE_EFUSE_MAP_LEN ,beacuse 8188E raw 256,logic map over
    // 256.
    EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI, TYPE_EFUSE_MAP_LEN,
                             (PVOID)&max_available_size, _FALSE);
#endif
...
// pidvid,da0b7881
    ...
    if (rtw_efuse_map_read(padapter, 0, EFUSE_MAP_SIZE,
                           pEfuseHal->fakeEfuseModifiedMap) == _SUCCESS) {
    ...
    *extra = 0;
    DBG_871X("%s: after rtw_BT_efuse_map_write to _rtw_memcmp \n",
             __FUNCTION__);
    if ((rtw_BT_efuse_map_read(padapter, addr, cnts, ShadowMapBT) ==
         _SUCCESS)) {
    ...
    for (jj = 0, kk = 0; jj < cnts; jj++, kk += 2) {
      pEfuseHal->fakeBTEfuseModifiedMap[addr + jj] =
          key_2char2num(tmp[2][kk], tmp[2][kk + 1]);
    }
  } else if (strcmp(tmp[0], "btdumpfake") == 0) {
    if (rtw_BT_efuse_map_read(padapter, 0, EFUSE_BT_MAX_MAP_LEN,
                              pEfuseHal->fakeBTEfuseModifiedMap) == _SUCCESS) {
    ...
    _rtw_memcpy(pEfuseHal->BTEfuseModifiedMap,
                pEfuseHal->fakeBTEfuseModifiedMap, EFUSE_BT_MAX_MAP_LEN);
    ...
    if (rtw_BT_efuse_map_write(padapter, 0x00, EFUSE_BT_MAX_MAP_LEN,
                               pEfuseHal->fakeBTEfuseModifiedMap) == _FAIL) {
      DBG_871X("%s: rtw_BT_efuse_map_write error!\n", __FUNCTION__);
      err = -EFAULT;
      goto exit;
    }
    ...
    if ((rtw_BT_efuse_map_read(padapter, 0x00, EFUSE_BT_MAX_MAP_LEN,
                               pEfuseHal->fakeBTEfuseInitMap) == _SUCCESS)) {
      if (_rtw_memcmp((void *)pEfuseHal->fakeBTEfuseModifiedMap,
                      (void *)pEfuseHal->fakeBTEfuseInitMap,
        ...
        sprintf(extra, "BT write map compare FAIL");
        if (rtw_BT_efuse_map_write(padapter, 0x00, EFUSE_BT_MAX_MAP_LEN,
                                   pEfuseHal->fakeBTEfuseModifiedMap) ==
            _FAIL) {
          DBG_871X("%s: rtw_BT_efuse_map_write compare error,retry = %d!\n",
                   __FUNCTION__, i);
        }

        if (rtw_BT_efuse_map_read(padapter, EFUSE_BT, EFUSE_BT_MAX_MAP_LEN,
                                  pEfuseHal->fakeBTEfuseInitMap) == _SUCCESS) {
        ...
        DBG_871X("%s: BT write map afterf compare not match to write efuse try "
                 "write Map again , BTStatus=0x%x\n",
                 __FUNCTION__, BTStatus);
        goto exit;
      }
    }
    ...
    if (rtw_efuse_map_write(padapter, 0x00, EFUSE_MAP_SIZE,
                            pEfuseHal->fakeEfuseModifiedMap) == _FAIL) {
    ...
    *extra = 0;
    DBG_871X("%s: after rtw_BT_efuse_map_write to _rtw_memcmp \n",
             __FUNCTION__);
    if ((rtw_efuse_map_read(padapter, 0x00, EFUSE_MAP_SIZE, ShadowMapWiFi) ==
         _SUCCESS)) {
      if (_rtw_memcmp((void *)ShadowMapWiFi, (void *)setdata, cnts)) {
    ...
    for (jj = 0, kk = 0; jj < cnts; jj++, kk += 2) {
      pEfuseHal->fakeEfuseModifiedMap[addr + jj] =
          key_2char2num(tmp[2][kk], tmp[2][kk + 1]);
    }
  }
 ...
 *	%d for data readed
 */
static int rtw_mp_read_reg(struct net_device *dev, struct iw_request_info *info,
 ...
 */
static int rtw_mp_write_rf(struct net_device *dev, struct iw_request_info *info,
                           struct iw_point *wrqu, char *extra) {
 ...
 *	%d for data readed
 */
static int rtw_mp_read_rf(struct net_device *dev, struct iw_request_info *info,
                          struct iw_point *wrqu, char *extra) {
  char input[wrqu->length];
  char *pch, *pnext, *ptmp;
  char data[20], tmp[20];
...
static int rtw_mp_stop(struct net_device *dev, struct iw_request_info *info,
                       struct iw_point *wrqu, char *extra) {
  PADAPTER padapter = rtw_netdev_priv(dev);
  struct hal_ops *pHalFunc = &padapter->HalFunc;
...
static int rtw_mp_rate(struct net_device *dev, struct iw_request_info *info,
                       struct iw_point *wrqu, char *extra) {
...
static int rtw_mp_channel(struct net_device *dev, struct iw_request_info *info,
                          struct iw_point *wrqu, char *extra) {
  ...
  _rtw_memset(extra, 0, wrqu->length);
  sprintf(extra, "Change channel %d to channel %d", padapter->mppriv.channel,
          channel);
...
static int rtw_mp_bandwidth(struct net_device *dev,
  ...
  // set_channel_bwmode(padapter, padapter->mppriv.channel, cur_ch_offset,
  // bandwidth);
...
static int rtw_mp_txpower(struct net_device *dev, struct iw_request_info *info,
                          struct iw_point *wrqu, char *extra) {
...
static int rtw_mp_ant_tx(struct net_device *dev, struct iw_request_info *info,
                         struct iw_point *wrqu, char *extra) {
  u8 i;
  u8 input[wrqu->length];
  u16 antenna = 0;
...
static int rtw_mp_ant_rx(struct net_device *dev, struct iw_request_info *info,
                         struct iw_point *wrqu, char *extra) {
  u8 i;
  u16 antenna = 0;
  u8 input[wrqu->length];
...
static int rtw_mp_ctx(struct net_device *dev, struct iw_request_info *info,
                      struct iw_point *wrqu, char *extra) {
  u32 pkTx = 1, countPkTx = 1, cotuTx = 1, CarrSprTx = 1, scTx = 1, sgleTx = 1,
      stop = 1;
  u32 bStartTest = 1;
  ...
  // DBG_871X("%s: count=%d countPkTx=%d cotuTx=%d CarrSprTx=%d scTx=%d
  // sgleTx=%d pkTx=%d stop=%d\n", __func__, count, countPkTx, cotuTx,
  // CarrSprTx, pkTx, sgleTx, scTx, stop);
...
static int rtw_mp_arx(struct net_device *dev, struct iw_request_info *info,
  ...
  bSetBssid =
      (strncmp(input, "setbssid=", 8) == 0) ? 1 : 0; // strncmp TRUE is 0
  ...
  if (bStartRx) {
    ...
    sprintf(extra, "Received packet OK:%d CRC error:%d ,Filter out:%d",
            padapter->mppriv.rx_pktcount, padapter->mppriv.rx_crcerrpktcount,
    ...
    if (IS_HARDWARE_TYPE_JAGUAR(padapter)) {
      ...
      OFDM_FA = PHY_QueryBBReg(padapter, 0xCF0, bMaskLWord) +
                PHY_QueryBBReg(padapter, 0xCF2, bMaskLWord) +
                PHY_QueryBBReg(padapter, 0xDA2, bMaskLWord) +
                PHY_QueryBBReg(padapter, 0xDA4, bMaskLWord) +
                PHY_QueryBBReg(padapter, 0xDA6, bMaskLWord) +
                PHY_QueryBBReg(padapter, 0xDA8, bMaskLWord);
    ...
    sprintf(extra, "Phy Received packet OK:%d CRC error:%d FA Counter: %d",
            cckok + ofdmok + htok + vht_ok, cckcrc + ofdmcrc + htcrc + vht_err,
            OFDM_FA + CCK_FA);
  } else if (bQueryMac) {
    // for 8723A
    {
      ...
      mac_vht_err = 0;
      // Mac_DropPacket
      rtw_write32(padapter, 0x664,
                  (rtw_read32(padapter, 0x0664) & 0x0FFFFFFF) | Mac_DropPacket);
      DropPacket = rtw_read32(padapter, 0x664) & 0x0000FFFF;
    }

    sprintf(extra,
            "Mac Received packet OK: %d , CRC error: %d , Drop Packets: %d\n",
            mac_cck_ok + mac_ofdm_ok + mac_ht_ok + mac_vht_ok,
...
static int rtw_mp_pwrtrk(struct net_device *dev, struct iw_request_info *info,
                         struct iw_point *wrqu, char *extra) {
  u8 enable;
  u32 thermal;
  s32 ret;
...
static int rtw_mp_psd(struct net_device *dev, struct iw_request_info *info,
                      struct iw_point *wrqu, char *extra) {
  PADAPTER padapter = rtw_netdev_priv(dev);
  u8 input[wrqu->length];
...
static int rtw_mp_thermal(struct net_device *dev, struct iw_request_info *info,
                          struct iw_point *wrqu, char *extra) {
  u8 val;
  int bwrite = 1;
...
static int rtw_mp_phypara(struct net_device *dev, struct iw_request_info *info,
                          struct iw_point *wrqu, char *extra) {
...
static int rtw_mp_QueryDrv(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
  PADAPTER padapter = rtw_netdev_priv(dev);
  char input[wrqu->data.length];
  s32 qAutoLoad = 1;
...
static int rtw_mp_PwrCtlDM(struct net_device *dev, struct iw_request_info *info,
                           struct iw_point *wrqu, char *extra) {
  PADAPTER padapter = rtw_netdev_priv(dev);
  u8 input[wrqu->length];
  int bstart = 1;
...
/* update Tx AGC offset */
static int rtw_mp_SetBT(struct net_device *dev, struct iw_request_info *info,
                        union iwreq_data *wrqu, char *extra) {
  PADAPTER padapter = rtw_netdev_priv(dev);
  struct hal_ops *pHalFunc = &padapter->HalFunc;
  HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
  ...
  u16 testmode = 1, ready = 1, trxparam = 1, setgen = 1, getgen = 1,
      testctrl = 1, testbt = 1, readtherm = 1, setbtmac = 1;
  ...
  if (strncmp(extra, "dlbt", 4) == 0) {
    ...
    DBG_8192C("FirmwareDownloadBT ready = 0x%x 0x%x", pMptCtx->mptOutBuf[4],
              pMptCtx->mptOutBuf[5]);
  ...
  if (ready != 0 && testmode != 0 && trxparam != 0 && setgen != 0 &&
      getgen != 0 && testctrl != 0 && testbt != 0 && readtherm != 0 &&
      setbtmac != 0)
    return -EFAULT;
  ...
  DBG_871X("%s: Req opCodeVer=%d OpCode=%d paraLength=%d\n", __FUNCTION__,
           BtReq.opCodeVer, BtReq.OpCode, BtReq.paraLength);
...
todo:
  ...
  if (readtherm == 0) {
    ...
    for (i = 4; i < pMptCtx->mptOutLen; i++) {
      if ((pMptCtx->mptOutBuf[i] == 0x00) &&
          (pMptCtx->mptOutBuf[i + 1] == 0x00))
        goto exit;
...
static int rtw_mp_set(struct net_device *dev, struct iw_request_info *info,
                      union iwreq_data *wdata, char *extra) {
  ...
  if ((padapter->bSurpriseRemoved == _TRUE) ||
    ...
    DBG_871X("%s fail =>(padapter->bSurpriseRemoved == _TRUE) || ( "
             "padapter->bDriverStopped == _TRUE) \n",
             __FUNCTION__);
    return -ENETDOWN;
  }
...
static int rtw_mp_get(struct net_device *dev, struct iw_request_info *info,
                      union iwreq_data *wdata, char *extra) {
  struct iw_point *wrqu = (struct iw_point *)wdata;
  u32 subcmd = wrqu->flags;
  PADAPTER padapter = rtw_netdev_priv(dev);
  ...
  rtw_msleep_os(10); // delay 5ms for sending pkt before exit adb shell
                     // operation
  return 0;
}
...
static int rtw_tdls_setup(struct net_device *dev, struct iw_request_info *info,
  ...
  if (_AES_ != padapter->securitypriv.dot11PrivacyAlgrthm) {
    ...
    if (0 == pwdinfo->wfd_tdls_weaksec) {
      ...
      DBG_871X("[%s] Current link is not AES, SKIP sending the tdls setup "
               "request!!\n",
               __FUNCTION__);
    } else {
  ...
  while (i < 4) {
    for (j = 0; j < 4; j++) {
      if (*(extra + j + tag) == '.' || *(extra + j + tag) == '\0') {
        if (j == 1)
          pwfd_info->ip_address[i] =
              convert_ip_addr('0', '0', *(extra + (j - 1) + tag));
        if (j == 2)
          pwfd_info->ip_address[i] = convert_ip_addr(
              '0', *(extra + (j - 2) + tag), *(extra + (j - 1) + tag));
        if (j == 3)
          pwfd_info->ip_address[i] = convert_ip_addr(*(extra + (j - 3) + tag),
                                                     *(extra + (j - 2) + tag),
                                                     *(extra + (j - 1) + tag));
...
static int rtw_tdls_getip(struct net_device *dev, struct iw_request_info *info,
  ...
  sprintf(extra, "\n\n%u.%u.%u.%u\n", pwfd_info->peer_ip_address[0],
          pwfd_info->peer_ip_address[1], pwfd_info->peer_ip_address[2],
          pwfd_info->peer_ip_address[3]);

  printk("[%s] IP=%u.%u.%u.%u\n", __FUNCTION__, pwfd_info->peer_ip_address[0],
         pwfd_info->peer_ip_address[1], pwfd_info->peer_ip_address[2],
         pwfd_info->peer_ip_address[3]);
...
static int rtw_tdls_getsta(struct net_device *dev, struct iw_request_info *info,
                           union iwreq_data *wrqu, char *extra) {
...
static int rtw_tdls(struct net_device *dev, struct iw_request_info *info,
                    union iwreq_data *wrqu, char *extra) {
  int ret = 0;
  ...
  DBG_871X("[%s] extra = %s\n", __FUNCTION__, extra);
  //	WFD Sigma will use the tdls enable command to let the driver know we
  //want to test the tdls now!
...
static int rtw_tdls_get(struct net_device *dev, struct iw_request_info *info,
                        union iwreq_data *wrqu, char *extra) {
  int ret = 0;
...
#ifdef CONFIG_INTEL_WIDI
static int rtw_widi_set(struct net_device *dev, struct iw_request_info *info,
                        union iwreq_data *wrqu, char *extra) {
  int ret = 0;
  _adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
...
extern void rtl8723a_fill_default_txdesc(struct xmit_frame *pxmitframe,
                                         u8 *pbuf);
...
extern void rtl8188es_fill_default_txdesc(struct xmit_frame *pxmitframe,
                                          u8 *pbuf);
...
extern void rtl8723b_fill_default_txdesc(struct xmit_frame *pxmitframe,
                                         u8 *pbuf);
...
static s32 createpseudoadhoc(PADAPTER padapter) {
...
#else
  {
    ...
    psta =
        rtw_get_stainfo(&padapter->stapriv, pcur_network->network.MacAddress);
    if (psta)
      rtw_free_stainfo(padapter, psta);
    ...
    _rtw_memcpy(&pcur_network->network, pdev_network,
...
static struct xmit_frame *createloopbackpkt(PADAPTER padapter, u32 size) {
  ...
  _rtw_memcpy(hdr->addr3, get_bssid(&padapter->mlmepriv),
              ETH_ALEN); // RA, BSSID
...
static u8 pktcmp(PADAPTER padapter, u8 *txbuf, u32 txsz, u8 *rxbuf, u32 rxsz) {
  ...
  if ((txsz - TXDESC_SIZE) != (rxpktsize - fcssize)) {
    DBG_8192C("%s: ERROR! size not match tx/rx=%d/%d !\n", __func__,
              txsz - TXDESC_SIZE, rxpktsize - fcssize);
    ret = _FALSE;
  } else {
    ret = _rtw_memcmp(txbuf + TXDESC_SIZE, rxbuf + RXDESC_SIZE + drvinfosize,
                      txsz - TXDESC_SIZE);
    if (ret == _FALSE) {
  ...
  if (ret == _FALSE) {
    ...
    DBG_8192C("\n%s: RX PKT read=%d offset=%d(%d,%d) content=%d\n", __func__,
              rxsz, RXDESC_SIZE + drvinfosize, RXDESC_SIZE, drvinfosize,
              rxpktsize);
    if (rxpktsize != 0) {
...
thread_return lbk_thread(thread_context context) {
  ...
  do {
    ...
    rtw_write_port(padapter, ff_hwaddr, ploopback->txsize,
                   (u8 *)pxmitframe->pxmitbuf);

    // wait for rx pkt
    _rtw_down_sema(&ploopback->sema);

    err = pktcmp(padapter, ploopback->txbuf, ploopback->txsize,
                 ploopback->rxbuf, ploopback->rxsize);
    if (err == _TRUE)
      ok++;
    else
      fail++;
    ...
    if ((ploopback->bstop == _TRUE) ||
      ...
      sprintf(ploopback->msg,
              "loopback result: ok=%d%%(%d/%d),error=%d%%(%d/%d)", ok_rate, ok,
              all, fail_rate, fail, all);
      break;
    }
  } while (1);
...
static const struct iw_priv_args rtw_private_args[] = {
  {SIOCIWFIRSTPRIV + 0x0, IW_PRIV_TYPE_CHAR | 0x7FF, 0, "write"},
  {SIOCIWFIRSTPRIV + 0x1, IW_PRIV_TYPE_CHAR | 0x7FF,
   IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_FIXED | IFNAMSIZ, "read"},
  {SIOCIWFIRSTPRIV + 0x2, 0, 0, "driver_ext"},
  {SIOCIWFIRSTPRIV + 0x3, 0, 0, "mp_ioctl"},
  {SIOCIWFIRSTPRIV + 0x4, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,
   "apinfo"},
  {SIOCIWFIRSTPRIV + 0x5, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 2, 0,
   "setpid"},
  ...
  {SIOCIWFIRSTPRIV + 0x7, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,
   "get_sensitivity"},
  {SIOCIWFIRSTPRIV + 0x8, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,
   "wps_prob_req_ie"},
  {SIOCIWFIRSTPRIV + 0x9, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,
   "wps_assoc_req_ie"},

  // for RTK_DMP_PLATFORM
  {SIOCIWFIRSTPRIV + 0xA, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0,
   "channel_plan"},

  {SIOCIWFIRSTPRIV + 0xB, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 2, 0, "dbg"},
  {SIOCIWFIRSTPRIV + 0xC, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 3, 0, "rfw"},
  {SIOCIWFIRSTPRIV + 0xD, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 2,
...
#endif
  {SIOCIWFIRSTPRIV + 0x10, IW_PRIV_TYPE_CHAR | 1024, 0, "p2p_set"},
  {SIOCIWFIRSTPRIV + 0x11, IW_PRIV_TYPE_CHAR | 1024,
   IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "p2p_get"},
  {SIOCIWFIRSTPRIV + 0x12, 0, 0, "NULL"},
  {SIOCIWFIRSTPRIV + 0x13, IW_PRIV_TYPE_CHAR | 64, IW_PRIV_TYPE_CHAR | 64,
   "p2p_get2"},
  {SIOCIWFIRSTPRIV + 0x14, IW_PRIV_TYPE_CHAR | 64, 0, "tdls"},
  {SIOCIWFIRSTPRIV + 0x15, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | 1024,
   "tdls_get"},
  {SIOCIWFIRSTPRIV + 0x16, IW_PRIV_TYPE_CHAR | 64, 0, "pm_set"},

  {SIOCIWFIRSTPRIV + 0x18, IW_PRIV_TYPE_CHAR | IFNAMSIZ, 0, "rereg_nd_name"},
#ifdef CONFIG_MP_INCLUDED
  {SIOCIWFIRSTPRIV + 0x1A, IW_PRIV_TYPE_CHAR | 1024, 0, "NULL"},
  {SIOCIWFIRSTPRIV + 0x1B, IW_PRIV_TYPE_CHAR | 128,
   IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "NULL"},
#else
  {SIOCIWFIRSTPRIV + 0x1A, IW_PRIV_TYPE_CHAR | 1024, 0, "efuse_set"},
  {SIOCIWFIRSTPRIV + 0x1B, IW_PRIV_TYPE_CHAR | 128,
   IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "efuse_get"},
#endif
  {SIOCIWFIRSTPRIV + 0x1D, IW_PRIV_TYPE_CHAR | 40, IW_PRIV_TYPE_CHAR | 0x7FF,
   "test"},

#ifdef CONFIG_INTEL_WIDI
  {SIOCIWFIRSTPRIV + 0x1E, IW_PRIV_TYPE_CHAR | 1024, 0, "widi_set"},
  {SIOCIWFIRSTPRIV + 0x1F, IW_PRIV_TYPE_CHAR | 128, 0, "widi_prob_req"},
#endif // CONFIG_INTEL_WIDI

#ifdef CONFIG_MP_INCLUDED
  {SIOCIWFIRSTPRIV + 0x0E, IW_PRIV_TYPE_CHAR | 1024, 0, ""}, // set
  {SIOCIWFIRSTPRIV + 0x0F, IW_PRIV_TYPE_CHAR | 1024,
   IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, ""}, // get
  /* --- sub-ioctls definitions --- */
  {MP_START, IW_PRIV_TYPE_CHAR | 1024, 0, "mp_start"}, // set
  {MP_PHYPARA, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_phypara"},                                    // get
  {MP_STOP, IW_PRIV_TYPE_CHAR | 1024, 0, "mp_stop"}, // set
  {MP_CHANNEL, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_channel"},                                              // get
  {MP_BANDWIDTH, IW_PRIV_TYPE_CHAR | 1024, 0, "mp_bandwidth"}, // set
  {MP_RATE, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_rate"}, // get
  {MP_RESET_STATS, IW_PRIV_TYPE_CHAR | 1024, 0, "mp_reset_stats"},
  {MP_QUERY, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_query"}, // get
  {READ_REG, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "read_reg"},
  {MP_RATE, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_rate"},
  {READ_RF, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "read_rf"},
  {MP_PSD, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_psd"},
  {MP_DUMP, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_dump"},
  {MP_TXPOWER, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_txpower"},
  {MP_ANT_TX, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_ant_tx"},
  {MP_ANT_RX, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_ant_rx"},
  {WRITE_REG, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "write_reg"},
  {WRITE_RF, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "write_rf"},
  {MP_CTX, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_ctx"},
  {MP_ARX, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_arx"},
  {MP_THER, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_ther"},
  {EFUSE_SET, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "efuse_set"},
  {EFUSE_GET, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "efuse_get"},
  {MP_PWRTRK, IW_PRIV_TYPE_CHAR | 1024, 0, "mp_pwrtrk"},
  {MP_QueryDrvStats, IW_PRIV_TYPE_CHAR | 1024,
   IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_drvquery"},
  {MP_IOCTL, IW_PRIV_TYPE_CHAR | 1024, 0, "mp_ioctl"}, // mp_ioctl
  {MP_SetRFPathSwh, IW_PRIV_TYPE_CHAR | 1024, 0, "mp_setrfpath"},
  {MP_PwrCtlDM, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_pwrctldm"},
  {MP_GET_TXPOWER_INX, IW_PRIV_TYPE_CHAR | 1024,
   IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_get_txpower"},

#if defined(CONFIG_RTL8723A) || defined(CONFIG_RTL8723B)
  {MP_SetBT, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
   "mp_setbt"},
...
static struct iw_statistics *rtw_get_wireless_stats(struct net_device *dev) {
  ...
  if (check_fwstate(&padapter->mlmepriv, _FW_LINKED) != _TRUE) {
...
#ifdef CONFIG_SKIP_SIGNAL_SCALE_MAPPING
    {
      ...
      tmp_level = (u8)odm_SignalScaleMapping(
          &pHal->odmpriv, padapter->recvpriv.signal_strength);
    }
#else
    ...
    if (rtw_linked_check(padapter)) {
      ...
      rtw_hal_get_odm_var(padapter, HAL_ODM_NOISE_MONITOR, &(info.chan),
    ...
    // tmp_noise,padapter->recvpriv.rssi);
...
#else
#ifdef RTK_DMP_PLATFORM
  // IW_QUAL_DBM= 0x8, if driver use this flag, wireless extension will show
  // value of dbm.
...
static int rtw_ioctl_wext_private(struct net_device *dev,
