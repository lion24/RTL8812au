...
sint _rtw_init_mlme_priv(_adapter *padapter) {
  ...
  // We don't need to memset padapter->XXX to zero, because adapter is allocated
  // by rtw_zvmalloc().
...
#define RTW_ROAM_SCAN_INTERVAL_MS 10 * 1000
  ...
  pmlmepriv->roam_flags = 0 | RTW_ROAM_ON_EXPIRED
...
_func_enter_;
        ...
        {
                pnetwork = LIST_CONTAINOR(get_next(&queue->queue), struct
wlan_network, list);
...
struct wlan_network *
_rtw_alloc_network(struct mlme_priv *pmlmepriv) //(_queue *free_queue)
{
  _irqL irqL;
  struct wlan_network *pnetwork;
...
u8 *rtw_get_timestampe_from_ie(u8 *ie) { return (ie + 0); }
...
struct wlan_network *
rtw_alloc_network(struct mlme_priv *pmlmepriv) //(_queue	*free_queue)
{
  struct wlan_network *pnetwork;
  _func_enter_;
...
void rtw_free_network(
    struct mlme_priv *pmlmepriv, struct wlan_network *pnetwork,
    u8 is_freeall) //(struct	wlan_network *pnetwork, _queue	*free_queue)
{
  _func_enter_;
...
void rtw_free_network_nolock(_adapter *padapter,
                             struct wlan_network *pnetwork) {
  _func_enter_;
...
int is_same_network(WLAN_BSSID_EX *src, WLAN_BSSID_EX *dst, u8 feature) {
  ...
  return (
      (src->Ssid.SsidLength == dst->Ssid.SsidLength) &&
      //	(src->Configuration.DSConfig == dst->Configuration.DSConfig) &&
      ((_rtw_memcmp(src->MacAddress, dst->MacAddress, ETH_ALEN)) == _TRUE) &&
      ((_rtw_memcmp(src->Ssid.Ssid, dst->Ssid.Ssid, src->Ssid.SsidLength)) ==
       _TRUE) &&
      ((s_cap & WLAN_CAPABILITY_IBSS) == (d_cap & WLAN_CAPABILITY_IBSS)) &&
      ((s_cap & WLAN_CAPABILITY_BSS) == (d_cap & WLAN_CAPABILITY_BSS)));
}

struct wlan_network *_rtw_find_same_network(_queue *scanned_queue,
...
struct wlan_network *rtw_find_same_network(_queue *scanned_queue,
                                           struct wlan_network *network) {
  _irqL irqL;
  struct wlan_network *found = NULL;
...
void update_network(WLAN_BSSID_EX *dst, WLAN_BSSID_EX *src, _adapter *padapter,
  ...
  if (check_fwstate(&padapter->mlmepriv, _FW_LINKED) &&
    ...
    /* the rssi value here is undecorated, and will be used for antenna
     * diversity */
    ...
    if (sq_smp != 101) { /* from the right channel */
      ss_final = ((u32)(src->PhyInfo.SignalStrength) +
                  (u32)(dst->PhyInfo.SignalStrength) * 4) /
                 5;
      sq_final = ((u32)(src->PhyInfo.SignalQuality) +
                  (u32)(dst->PhyInfo.SignalQuality) * 4) /
                 5;
      rssi_final = (src->Rssi + dst->Rssi * 4) / 5;
    } else {
      /* bss info not receving from the right channel, use the original RX
       * signal infos */
  ...
  if (strcmp(dst->Ssid.Ssid, DBG_RX_SIGNAL_DISPLAY_SSID_MONITORED) == 0) {
    DBG_871X(
        FUNC_ADPT_FMT " %s(" MAC_FMT
                      "), SignalStrength:%u, SignalQuality:%u, RawRSSI:%ld\n",
        FUNC_ADPT_ARG(padapter), dst->Ssid.Ssid, MAC_ARG(dst->MacAddress),
        dst->PhyInfo.SignalStrength, dst->PhyInfo.SignalQuality, dst->Rssi);
  }
#endif
...
void rtw_update_scanned_network(_adapter *adapter, WLAN_BSSID_EX *target) {
  ...
  while (1) {
    ...
    if (!rtw_p2p_chk_state(&(adapter->wdinfo), P2P_STATE_NONE) &&
        (_rtw_memcmp(pnetwork->network.MacAddress, target->MacAddress,
                     ETH_ALEN) == _TRUE)) {
      target_find = 1;
      break;
    }
    ...
    if (rtw_roam_flags(adapter)) {
      /* TODO: don't  select netowrk in the same ess as oldest if it's new
       * enough*/
    }
  ...
  if (!target_find) {
    if (_rtw_queue_empty(&(pmlmepriv->free_bss_pool)) == _TRUE) {
      ...
      // pHalData->CurAntenna;//optimum_antenna=>For antenna diversity
...
int rtw_is_desired_network(_adapter *adapter, struct wlan_network *pnetwork) {
      ...
      1) // for  correct flow of 8021X  to do....
  {
    ...
    if (psecuritypriv->ndisauthtype == Ndis802_11AuthModeWPA2PSK) {
      p = rtw_get_ie(pnetwork->network.IEs + _BEACON_IE_OFFSET_, _RSN_IE_2_,
                     &ie_len,
                     (pnetwork->network.IELength - _BEACON_IE_OFFSET_));
      if (p && ie_len > 0) {
        bselected = _TRUE;
      } else {
        bselected = _FALSE;
...
void rtw_survey_event_callback(_adapter *adapter, u8 *pbuf) {
  ...
  if ((check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE)) == _TRUE) {
    // RT_TRACE(_module_rtl871x_mlme_c_,_drv_err_,"rtw_survey_event_callback :
    // WIFI_ADHOC_MASTER_STATE \n\n");
    if (_rtw_memcmp(&(pmlmepriv->cur_network.network.MacAddress),
      ...
      ibss_wlan =
          rtw_find_network(&pmlmepriv->scanned_queue, pnetwork->MacAddress);
      if (ibss_wlan) {
...
void rtw_surveydone_event_callback(_adapter *adapter, u8 *pbuf) {
    ...
    if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE)) {
      if (check_fwstate(pmlmepriv, _FW_LINKED) == _FALSE) {
        ...
        if (rtw_select_and_join_from_scanned_queue(pmlmepriv) == _SUCCESS) {
          ...
          // pmlmepriv->fw_state ^= _FW_UNDER_SURVEY;//because don't set
          // assoc_timer
          _clr_fwstate_(pmlmepriv, _FW_UNDER_SURVEY);
          ...
          _rtw_memcpy(&pdev_network->Ssid, &pmlmepriv->assoc_ssid,
  ...
  if (adapter->pbuddy_adapter) {
    ...
    if (buddy_wdev_priv->scan_request &&
        buddy_mlme->scanning_via_buddy_intf == _TRUE) {
...
void rtw_free_assoc_resources(_adapter *adapter, int lock_scanned_queue) {
  ...
  if (pwlan) {
    ...
    if (!rtw_p2p_chk_state(&adapter->wdinfo, P2P_STATE_NONE)) {
      ...
      if ((p2p_ie = rtw_get_p2p_ie(pwlan->network.IEs + _FIXED_IE_LENGTH_,
                                   pwlan->network.IELength - _FIXED_IE_LENGTH_,
                                   NULL, &p2p_ielen))) {
        pcap = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CAPABILITY,
                                        NULL, &capability_len);
        if (pcap && capability_len == 2) {
          u16 cap = *(u16 *)pcap;
          *(u16 *)pcap =
              cap & 0x00ff; // clear group capability when free this network
        }
      }
...
void rtw_indicate_disconnect(_adapter *padapter) {
  ...
  if (pmlmepriv->cur_network_scanned) {
    WLAN_BSSID_EX *current_joined_bss =
        &(pmlmepriv->cur_network_scanned->network);
    if (current_joined_bss) {
      wps_ie = rtw_get_wps_ie(current_joined_bss->IEs + _FIXED_IE_LENGTH_,
                              current_joined_bss->IELength - _FIXED_IE_LENGTH_,
                              NULL, &wpsie_len);
  ...
  beamforming_wk_cmd(padapter, BEAMFORMING_CTRL_LEAVE, cur_network->MacAddress,
                     ETH_ALEN, 1);
#endif
...
inline void rtw_indicate_scan_done(_adapter *padapter, bool aborted) {
  ...
  if (is_primary_adapter(padapter) &&
      ...
      (check_fwstate(&padapter->mlmepriv,
                     WIFI_ASOC_STATE | WIFI_UNDER_LINKING) == _FALSE)) {
...
rtw_joinbss_update_stainfo(_adapter *padapter, struct wlan_network *pnetwork) {
  int i;
  ...
  if (psta) // update ptarget_sta
  {
...
#if 0 // alloc macid when call rtw_alloc_stainfo(), and release macid when call
      // rtw_free_stainfo()
#ifdef CONFIG_CONCURRENT_MODE	
    ...
    _rtw_memcpy(psta->bssrateset, pnetwork->network.SupportedRates,
                psta->bssratelen);
    ...
    for (i = 0; i < 16; i++) {
      ...
      preorder_ctrl->wsize_b = 64; // max_ampdu_sz;//ex. 32(kbytes) ->
                                   // wsize_b=32
    }
...
static void rtw_joinbss_update_network(_adapter *padapter,
  ...
  _rtw_memcpy(&cur_network->network, &pnetwork->network,
              pnetwork->network.Length);
  // some IEs in pnetwork is wrong, so we should use ptarget_wlan IEs
  cur_network->network.IELength = ptarget_wlan->network.IELength;
  _rtw_memcpy(&cur_network->network.IEs[0], &ptarget_wlan->network.IEs[0],
              MAX_IE_SZ);
  ...
  padapter->recvpriv.rssi =
      translate_percentage_to_dbm(ptarget_wlan->network.PhyInfo.SignalStrength);
#if defined(DBG_RX_SIGNAL_DISPLAY_PROCESSING) && 1
  DBG_871X(FUNC_ADPT_FMT " signal_strength:%3u, rssi:%3d, signal_qual:%3u"
                         "\n",
           FUNC_ADPT_ARG(padapter), padapter->recvpriv.signal_strength,
  ...
  rtw_update_protection(
      padapter, (cur_network->network.IEs) + sizeof(NDIS_802_11_FIXED_IEs),
...
// "ptarget_wlan" exist.
// if join_res > 0, update "cur_network->network" from "pnetwork->network" if
// (ptarget_wlan !=NULL).
//
//#define REJOIN
void rtw_joinbss_event_prehandle(_adapter *adapter, u8 *pbuf) {
  ...
  the_same_macaddr = _rtw_memcmp(pnetwork->network.MacAddress,
  ...
  if (pnetwork->join_res > 0) {
    ...
    if (check_fwstate(pmlmepriv, _FW_UNDER_LINKING)) {
      ...
      if (check_fwstate(pmlmepriv, _FW_LINKED)) {
        ...
        ptarget_wlan =
            _rtw_find_same_network(&pmlmepriv->scanned_queue, pnetwork);
        if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE) {
          if (ptarget_wlan)
            ptarget_wlan->fixed = _TRUE;
        }
      ...
      // s3. find ptarget_sta & update ptarget_sta after update cur_network only
      // for station mode
      if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE) {
...
void rtw_sta_media_status_rpt(_adapter *adapter, struct sta_info *psta,
                              u32 mstatus) {
...
#endif
  media_status_rpt =
      (u16)((psta->mac_id << 8) | mstatus); //  MACID|OPMODE:1 connect
...
void rtw_stassoc_event_callback(_adapter *adapter, u8 *pbuf) {
...
#ifdef CONFIG_BEAMFORMING
      beamforming_wk_cmd(adapter, BEAMFORMING_CTRL_ENTER, (u8 *)psta,
                         sizeof(struct sta_info), 0);
#endif
    }
    goto exit;
  }
  ...
  if (psta != NULL) {
    ...
    RT_TRACE(_module_rtl871x_mlme_c_, _drv_err_,
             ("Error: rtw_stassoc_event_callback: sta has been in "
              "sta_hash_queue \n"));

    goto exit; //(between drv has received this event before and  fw have not
               //yet to set key to CAM_ENTRY)
  }
...
void rtw_stadel_event_callback(_adapter *adapter, u8 *pbuf) {
  ...
  if ((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) {
    ...
    defined(CONFIG_CFG80211_FORCE_COMPATIBLE_2_6_37_UNDER)
  ...
  if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
    ...
    if (roam == _TRUE) {
      if (rtw_to_roam(adapter) > 0)
        rtw_dec_to_roam(adapter); /* this stadel_event is caused by roaming,
                                     decrease to_roam */
  ...
  if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) ||
        ...
        1) // a sta + bc/mc_stainfo (not Ibss_stainfo)
    {
      ...
      _rtw_memcpy(pdev_network, &tgt_network->network,
      ...
      if (rtw_createbss_cmd(adapter) != _SUCCESS) {
        ...
        RT_TRACE(_module_rtl871x_ioctl_set_c_, _drv_err_,
                 ("***Error=>stadel_event_callback: rtw_createbss_cmd status "
                  "FAIL*** \n "));
      }
    }
  }
...
#ifdef CONFIG_IOCTL_CFG80211
    // indicate disconnect for the case that join_timeout and check_fwstate !=
    // FW_LINKED
  ...
  if (pmlmepriv->auto_scan_int_ms != 0 &&
    ...
    if (rtw_buddy_adapter_up(padapter)) {
      if ((check_buddy_fwstate(
               padapter, _FW_UNDER_SURVEY | _FW_UNDER_LINKING) == _TRUE) ||
          (padapter->pbuddy_adapter->mlmepriv.LinkDetectInfo.bBusyTraffic ==
           _TRUE)) {
        DBG_871X(
            FUNC_ADPT_FMT
            ", but buddy_intf is under scanning or linking or BusyTraffic\n",
            FUNC_ADPT_ARG(padapter));
        goto exit;
      }
    }
...
void rtw_dynamic_check_timer_handlder(_adapter *adapter) {
  ...
  if (is_primary_adapter(adapter))
    DBG_871X("IsBtDisabled=%d, IsBtControlLps=%d\n",
             rtw_btcoex_IsBtDisabled(adapter),
             rtw_btcoex_IsBtControlLps(adapter));
#endif
...
#ifdef CONFIG_LAYER2_ROAMING
/*
* Select a new roaming candidate from the original @param candidate and @param
* competitor
...
static int rtw_check_roaming_candidate(struct mlme_priv *mlme,
  ...
  DBG_871X(
      "roam candidate:%s %s(" MAC_FMT ", ch%3u) rssi:%d, age:%5d\n",
      (competitor == mlme->cur_network_scanned) ? "*" : " ",
      competitor->network.Ssid.Ssid, MAC_ARG(competitor->network.MacAddress),
      competitor->network.Configuration.DSConfig, (int)competitor->network.Rssi,
      rtw_get_passing_time_ms(competitor->last_scanned));

  /* got specific addr to roam */
  if (!is_zero_mac_addr(mlme->roam_tgt_addr)) {
    if (_rtw_memcmp(mlme->roam_tgt_addr, competitor->network.MacAddress,
                    ETH_ALEN) == _TRUE)
      goto update;
    else
      goto exit;
...
int rtw_select_roaming_candidate(struct mlme_priv *mlme) {
  ...
  if (candidate == NULL) {
    ...
    if (_rtw_memcmp(candidate->network.MacAddress, mlme->roam_tgt_addr,
                    ETH_ALEN) == _TRUE)
...
/*
* Select a new join candidate from the original @param candidate and @param
* competitor
* @return _TRUE: candidate is updated
* @return _FALSE: candidate is not updated
*/
static int rtw_check_join_candidate(struct mlme_priv *mlme,
  ...
  if (mlme->assoc_by_bssid == _TRUE) {
    if (_rtw_memcmp(competitor->network.MacAddress, mlme->assoc_bssid,
                    ETH_ALEN) == _FALSE)
      goto exit;
  }
  ...
  if (updated) {
    DBG_871X("[by_bssid:%u][assoc_ssid:%s]"
             ...
             mlme->assoc_by_bssid, mlme->assoc_ssid.Ssid,
#ifdef CONFIG_LAYER2_ROAMING
             rtw_to_roam(adapter),
#endif
             (*candidate)->network.Ssid.Ssid,
...
#if 0 // for WPA/WPA2 authentication, wpa_supplicant will expect authentication
      // from AP, it is needed to reconnect AP...
...
sint rtw_set_key(_adapter *adapter, struct security_priv *psecuritypriv,
  ...
  if (psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_8021X) {
    ...
    RT_TRACE(
        _module_rtl871x_mlme_c_, _drv_err_,
        ("\n rtw_set_key: "
         "psetkeyparm->algorithm=(u8)psecuritypriv->dot11PrivacyAlgrthm=%d \n",
         psetkeyparm->algorithm));
  }
  ...
  DBG_871X("==> rtw_set_key algorithm(%x),keyid(%x),key_mask(%x)\n",
           ...
           adapter->securitypriv.key_mask);
  RT_TRACE(_module_rtl871x_mlme_c_, _drv_err_,
           ("\n rtw_set_key: psetkeyparm->algorithm=%d "
            "psetkeyparm->keyid=(u8)keyid=%d \n",
  ...
  case _TKIP_:
    keylen = 16;
    _rtw_memcpy(&psetkeyparm->key, &psecuritypriv->dot118021XGrpKey[keyid],
                keylen);
  ...
  default:
    RT_TRACE(_module_rtl871x_mlme_c_, _drv_err_,
             ("\n rtw_set_key:psecuritypriv->dot11PrivacyAlgrthm = %x (must be "
              "1 or 2 or 4 or 5)\n",
...
int rtw_restruct_wmm_ie(_adapter *adapter, u8 *in_ie, u8 *out_ie, uint in_len,
  ...
  while (i < in_len) {
    ...
    if (in_ie[i] == 0xDD && in_ie[i + 2] == 0x00 && in_ie[i + 3] == 0x50 &&
        ...
        i + 5 < in_len) // WMM element ID and OUI
    {
...
//
// Ported from 8185: IsInPreAuthKeyList(). (Renamed from
// SecIsInPreAuthKeyList(), 2006-10-13.)
// Added by Annie, 2006-05-07.
//
// Search by BSSID,
// Return Value:
//		-1 		:if there is no pre-auth key in the  table
//		>=0		:if there is pre-auth key, and   return the entry
//id
//
//
...
static int rtw_append_pmkid(_adapter *adapter, int iEntry, u8 *ie,
                            uint ie_len) {
  ...
  if (ie[13] > 20) {
    ...
    if (pmkid_cnt == 1 &&
        _rtw_memcmp(ie + 14 + 20 + 2, &sec->PMKIDList[iEntry].PMKID, 16)) {
...
sint rtw_restruct_sec_ie(_adapter *adapter, u8 *in_ie, u8 *out_ie,
  ...
  if (check_fwstate(pmlmepriv, WIFI_UNDER_WPS)) {
    _rtw_memcpy(out_ie + ielength, psecuritypriv->wps_ie,
                psecuritypriv->wps_ie_len);

    ielength += psecuritypriv->wps_ie_len;
  } else if ((authmode == _WPA_IE_ID_) || (authmode == _WPA2_IE_ID_)) {
    // copy RSN or SSN
    _rtw_memcpy(&out_ie[ielength], &psecuritypriv->supplicant_ie[0],
...
void rtw_init_registrypriv_dev_network(_adapter *adapter) {
  ...
  _rtw_memcpy(&pdev_network->Ssid, &pregistrypriv->ssid,
              sizeof(NDIS_802_11_SSID));
...
void rtw_update_registrypriv_dev_network(_adapter *adapter) {
  ...
  // notes: translate IELength & Length after assign the Length to cmdsz in
  // createbss_cmd();
...
void rtw_joinbss_reset(_adapter *padapter) {
  u8 threshold;
  struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
// todo: if you want to do something io/reg/hw setting before join_bss, please
// add code here
...
void rtw_build_wmm_ie_ht(_adapter *padapter, u8 *out_ie, uint *pout_len) {
  ...
  if (padapter->mlmepriv.qospriv.qos_option == 0) {
    out_len = *pout_len;
    pframe = rtw_set_ie(out_ie + out_len, _VENDOR_SPECIFIC_IE_, _WMM_IE_Length_,
                        WMM_IE, pout_len);
...
unsigned int rtw_restructure_ht_ie(_adapter *padapter, u8 *in_ie, u8 *out_ie,
  ...
  if (TEST_FLAG(phtpriv->stbc_cap, STBC_HT_ENABLE_RX)) {
    if ((pregistrypriv->rx_stbc == 0x3) || /* enable for 2.4/5 GHz */
        ...
        ((channel > 14) &&
         (pregistrypriv->rx_stbc == 0x2)) || /* enable for 5GHz */
  ...
  default:
    ...
    if (stbc_rx_enable)
      ht_capie.cap_info |=
          IEEE80211_HT_CAP_RX_STBC_2R; // RX STBC two spatial stream
  ...
  */
  ...
  if (padapter->driver_rx_ampdu_spacing != 0xFF) {
    ht_capie.ampdu_params_info |=
        ((padapter->driver_rx_ampdu_spacing & 0x07) << 2);
  } else {
  ...
  if (in_ie != NULL) {
    ...
    if (p && (ielen == sizeof(struct ieee80211_ht_addt_info))) {
      out_len = *pout_len;
      pframe = rtw_set_ie(out_ie + out_len, _HT_ADD_INFO_IE_, ielen, p + 2,
                          pout_len);
    }
  }
...
void rtw_update_ht_cap(_adapter *padapter, u8 *pie, uint ie_len, u8 channel) {
  ...
  len = 0;
  p = rtw_get_ie(pie + sizeof(NDIS_802_11_FIXED_IEs), _HT_CAPABILITY_IE_, &len,
                 ie_len - sizeof(NDIS_802_11_FIXED_IEs));
  if (p && len > 0) {
    pht_capie = (struct rtw_ieee80211_ht_cap *)(p + 2);
    max_ampdu_sz =
        (pht_capie->ampdu_params_info & IEEE80211_HT_CAP_AMPDU_FACTOR);
  ...
  len = 0;
  p = rtw_get_ie(pie + sizeof(NDIS_802_11_FIXED_IEs), _HT_ADD_INFO_IE_, &len,
                 ie_len - sizeof(NDIS_802_11_FIXED_IEs));
  if (p && len > 0) {
...
#ifdef CONFIG_TDLS
void rtw_issue_addbareq_cmd_tdls(_adapter *padapter,
...
void rtw_append_exented_cap(_adapter *padapter, u8 *out_ie, uint *pout_len) {
  ...
  pframe = rtw_set_ie(out_ie + *pout_len, EID_EXTCapability, 8, cap_content,
                      pout_len);
}
#endif
...
inline u8 rtw_to_roam(_adapter *adapter) { return adapter->mlmepriv.to_roam; }
...
void _rtw_roaming(_adapter *padapter, struct wlan_network *tgt_network) {
  ...
  if (0 < rtw_to_roam(padapter)) {
    ...
    _rtw_memcpy(&pmlmepriv->assoc_ssid, &cur_network->network.Ssid,
                sizeof(NDIS_802_11_SSID));
...
sint rtw_linked_check(_adapter *padapter) {
  if ((check_fwstate(&padapter->mlmepriv, WIFI_AP_STATE) == _TRUE) ||
      (check_fwstate(&padapter->mlmepriv,
                     WIFI_ADHOC_STATE | WIFI_ADHOC_MASTER_STATE) == _TRUE)) {
...
sint rtw_buddy_adapter_up(_adapter *padapter) {
  ...
  if (padapter->pbuddy_adapter == NULL) {
    res = _FALSE;
             ...
             (padapter->pbuddy_adapter->bup == _FALSE) ||
