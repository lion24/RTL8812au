...
// offset 20
    ...
    if ((pattrib->ether_type != 0x888e) && (pattrib->ether_type != 0x0806) &&
        (pattrib->ether_type != 0x88b4) && (pattrib->dhcp_pkt != 1)
      ...
      if (pHalData->fw_ractrl == _FALSE) {
        ...
        SET_TX_DESC_TX_RATE_8812(
            ptxdesc, (pdmpriv->INIDATA_RATE[pattrib->mac_id] & 0x7F));
      }
...
s32 rtl8812au_xmit_buf_handler(PADAPTER padapter) {
  ...
  ret = (padapter->bDriverStopped == _TRUE) ||
  ...
  do {
    ...
    rtw_write_port(padapter, pxmitbuf->ff_hwaddr, pxmitbuf->len,
...
static u32 xmitframe_need_length(struct xmit_frame *pxmitframe) {
  ...
  len = pattrib->hdrlen + pattrib->iv_len + SNAP_SIZE + sizeof(u16) +
        pattrib->pktlen + ((pattrib->bswenc) ? pattrib->icv_len : 0);
...
s32 rtl8812au_xmitframe_complete(_adapter *padapter,
                                 struct xmit_priv *pxmitpriv,
                                 struct xmit_buf *pxmitbuf) {
  HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
  ...
  do {
    ...
    pxmitframe->pkt_offset =
        (PACKET_OFFSET_SZ / 8) + 1; // 2; // first frame of aggregation, reserve
                                    // one offset for EM info ,another for usb
                                    // bulk-out block check
#else
    pxmitframe->pkt_offset =
        (PACKET_OFFSET_SZ /
         8); // 1; // first frame of aggregation, reserve offset
#endif

    if (rtw_xmitframe_coalesce(padapter, pxmitframe->pkt, pxmitframe) ==
        _FALSE) {
  ...
  // DBG_8192C("==>
  // pkt_no=%d,pkt_len=%d,len=%d,RND8_LEN=%d,pkt_offset=0x%02x\n",
  ...
  while (rtw_end_of_queue_search(xmitframe_phead, xmitframe_plist) == _FALSE) {
    ...
    if (_FAIL == rtw_hal_busagg_qsel_check(padapter, pfirstframe->attrib.qsel,
                                           pxmitframe->attrib.qsel))
      break;

    pxmitframe->agg_num = 0; // not first frame of aggregation
#ifdef CONFIG_TX_EARLY_MODE
    pxmitframe->pkt_offset =
        1; // not first frame of aggregation,reserve offset for EM Info
#else
    pxmitframe->pkt_offset =
    ...
    // if (_RND8(pbuf + len) > (MAX_XMITBUF_SZ/2))//to do : for TX TP finial
    // tune , Georgia 2012-0323
    {
    ...
    update_txdesc(pxmitframe, pxmitframe->buf_addr,
                  pxmitframe->attrib.last_txcmdsz, _TRUE);
    ...
    pxmitpriv->agg_pkt[pfirstframe->agg_num - 1].pkt_len =
...
agg_end:
  ...
  update_txdesc(pfirstframe, pfirstframe->buf_addr,
                pfirstframe->attrib.last_txcmdsz, _TRUE);

#ifdef CONFIG_TX_EARLY_MODE
  // prepare EM info for first frame, agg_num value start from 1
  pxmitpriv->agg_pkt[0].offset =
      _RND8(pfirstframe->attrib.last_txcmdsz + TXDESC_SIZE +
            (pfirstframe->pkt_offset * PACKET_OFFSET_SZ));
  pxmitpriv->agg_pkt[0].pkt_len =
      pfirstframe->attrib.last_txcmdsz; // get from rtw_xmitframe_coalesce
  ...
  do {
    ...
    if (pxmitframe) {
      ...
      if ((pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG) {
        ...
        // DBG_8192C("==>
        // pxmitframe->attrib.priority:%d\n",pxmitframe->attrib.priority);
        rtw_os_xmit_complete(padapter, pxmitframe); // always return ndis_packet
                                                    // after
...
s32 rtl8812au_hal_xmitframe_enqueue(_adapter *padapter,
...
s32 rtl8812au_hostap_mgnt_xmit_entry(_adapter *padapter, _pkt *pkt) {
  ...
  // offset 12
  ptxdesc->txdw3 |=
      cpu_to_le32((le16_to_cpu(tx_hdr->seq_ctl) << 16) & 0xffff0000);
  ...
  ptxdesc->txdw3 |= cpu_to_le32(
      (8 << 28)); // set bit3 to 1. Suugested by TimChen. 2009.12.29.
  ...
  usb_fill_bulk_urb(urb, pdvobj->pusbdev, pipe, pxmit_skb->data, pxmit_skb->len,
                    rtl8192cu_hostap_mgnt_xmit_cb, pxmit_skb);
