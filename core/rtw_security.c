...
*/
void rtw_wep_encrypt(_adapter *padapter, u8 *pxmitframe) { // exclude ICV
  ...
  hw_hdr_offset = TXDESC_SIZE + (((struct xmit_frame *)pxmitframe)->pkt_offset *
                                 PACKET_OFFSET_SZ);
#else
  ...
  if ((pattrib->encrypt == _WEP40_) || (pattrib->encrypt == _WEP104_)) {
    ...
    for (curfragnum = 0; curfragnum < pattrib->nr_frags; curfragnum++) {
      ...
      _rtw_memcpy(
          &wepkey[3],
          &psecuritypriv->dot11DefKey[psecuritypriv->dot11PrivacyKeyIndex]
               .skey[0],
          keylength);
...
void rtw_wep_decrypt(_adapter *padapter, u8 *precvframe) {
  ...
  if ((prxattrib->encrypt == _WEP40_) || (prxattrib->encrypt == _WEP104_)) {
    ...
    _rtw_memcpy(&wepkey[0], iv, 3);
    //_rtw_memcpy(&wepkey[3],
    //&psecuritypriv->dot11DefKey[psecuritypriv->dot11PrivacyKeyIndex].skey[0],keylength);
    _rtw_memcpy(&wepkey[3], &psecuritypriv->dot11DefKey[keyindex].skey[0],
                keylength);
    length = ((union recv_frame *)precvframe)->u.hdr.len - prxattrib->hdrlen -
             prxattrib->iv_len;
    ...
    if (crc[3] != payload[length - 1] || crc[2] != payload[length - 2] ||
      ...
      RT_TRACE(_module_rtl871x_security_c_, _drv_err_,
               ("rtw_wep_decrypt:icv error crc[3](%x)!=payload[length-1](%x) "
                ...
                crc[3], payload[length - 1], crc[2], payload[length - 2],
                crc[1], payload[length - 3], crc[0], payload[length - 4]));
    }
...
void rtw_secmicappendbyte(struct mic_data *pmicdata, u8 b) {
  ...
  if (pmicdata->nBytesInM >= 4) {
    ...
    pmicdata->R ^=
        ((pmicdata->L & 0xff00ff00) >> 8) | ((pmicdata->L & 0x00ff00ff) << 8);
    pmicdata->L += pmicdata->R;
...
static void phase1(u16 *p1k, const u8 *tk, const u8 *ta, u32 iv32) {
  ...
  for (i = 0; i < PHASE1_LOOP_CNT;
       i++) { /* Each add operation here is mod 2**16 */
...
u32 rtw_tkip_encrypt(_adapter *padapter, u8 *pxmitframe) { // exclude ICV
  u16 pnl;
  u32 pnh;
  u8 rc4key[16];
  u8 ttkey[16];
  ...
  if (pattrib->encrypt == _TKIP_) {
    ...
    */
    // if (stainfo!=NULL)
    {
      ...
      */
      ...
      if (IS_MCAST(pattrib->ra)) {
        prwskey =
            psecuritypriv->dot118021XGrpKey[psecuritypriv->dot118021XGrpKeyid]
                .skey;
      } else {
    ...
    /*
                            ...
                            RT_TRACE(_module_rtl871x_security_c_,_drv_err_,("rtw_tkip_encrypt:
       stainfo==NULL!!!\n"));
                            DBG_871X("%s, psta==NUL\n", __func__);
                            res=_FAIL;
                    }
...
// The hlen isn't include the IV
u32 rtw_tkip_decrypt(_adapter *padapter, u8 *precvframe) { // exclude ICV
  u16 pnl;
  u32 pnh;
  u8 rc4key[16];
  u8 ttkey[16];
...
static void construct_mic_iv(
    u8 *mic_header1, sint qc_exists, sint a4_exists, u8 *mpdu,
    uint payload_length, u8 *pn_vector,
    uint frtype); // add for CONFIG_IEEE80211W, none 11w also can use
static void construct_mic_header1(
    u8 *mic_header1, sint header_length, u8 *mpdu,
    uint frtype); // add for CONFIG_IEEE80211W, none 11w also can use
static void construct_mic_header2(u8 *mic_header2, u8 *mpdu, sint a4_exists,
                                  sint qc_exists);
static void construct_ctr_preload(
    u8 *ctr_preload, sint a4_exists, sint qc_exists, u8 *mpdu, u8 *pn_vector,
    sint c,
    uint frtype); // add for CONFIG_IEEE80211W, none 11w also can use
...
static u8 sbox(u8 a) { return sbox_table[(sint)a]; }

static void next_key(u8 *key, sint round) {
  u8 rcon;
  u8 sbox_key[4];
  u8 rcon_table[12] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20,
...
/************************************************/
static void
construct_mic_iv(u8 *mic_iv, sint qc_exists, sint a4_exists, u8 *mpdu,
                 uint payload_length, u8 *pn_vector,
                 uint frtype // add for CONFIG_IEEE80211W, none 11w also can use
                 ) {
  sint i;
...
#ifdef CONFIG_IEEE80211W
  // 802.11w management frame don't AND subtype bits 4,5,6 of frame control
  // field
  if (frtype == WIFI_MGT_TYPE)
...
static sint aes_cipher(u8 *key, uint hdrlen, u8 *pframe, uint plen) {
  //	/*static*/ unsigned char	message[MAX_MSG_SIZE];
  uint qc_exists, a4_exists, i, j, payload_remainder, num_blocks, payload_index;
  ...
  construct_mic_iv(mic_iv, qc_exists, a4_exists,
                   pframe, // message,
                   plen, pn_vector,
  ...
  for (i = 0; i < num_blocks; i++) {
    bitwise_xor(aes_out, &pframe[payload_index],
                chain_buffer); // bitwise_xor(aes_out, &message[payload_index],
                               // chain_buffer);
  ...
  if (payload_remainder > 0) {
    ...
    for (j = 0; j < payload_remainder; j++) {
      padded_buffer[j] = pframe[payload_index++]; // padded_buffer[j] =
                                                  // message[payload_index++];
    }
  ...
  for (i = 0; i < num_blocks; i++) {
    construct_ctr_preload(
        ctr_preload, a4_exists, qc_exists,
        pframe, // message,
        pn_vector, i + 1,
        frtype); // add for CONFIG_IEEE80211W, none 11w also can use
    ...
    bitwise_xor(aes_out, &pframe[payload_index],
                chain_buffer); // bitwise_xor(aes_out, &message[payload_index],
                               // chain_buffer);
    for (j = 0; j < 16; j++)
      pframe[payload_index++] = chain_buffer[j]; // for (j=0; j<16;j++)
                                                 // message[payload_index++] =
                                                 // chain_buffer[j];
  }
  ...
  {                          /* encrypt it and copy the unpadded part back   */
    ...
    for (j = 0; j < payload_remainder; j++) {
      padded_buffer[j] =
          pframe[payload_index +
                 j]; // padded_buffer[j] = message[payload_index+j];
    }
    aes128k128d(key, ctr_preload, aes_out);
    bitwise_xor(aes_out, padded_buffer, chain_buffer);
    for (j = 0; j < payload_remainder; j++)
      pframe[payload_index++] =
          chain_buffer[j]; // for (j=0; j<payload_remainder;j++)
  ...
  for (j = 0; j < 8; j++) {
    padded_buffer[j] =
        pframe[j + hdrlen + 8 +
               plen]; // padded_buffer[j] = message[j+hdrlen+8+plen];
  }

  aes128k128d(key, ctr_preload, aes_out);
  bitwise_xor(aes_out, padded_buffer, chain_buffer);
  for (j = 0; j < 8; j++)
    pframe[payload_index++] = chain_buffer[j]; // for (j=0; j<8;j++)
                                               // message[payload_index++] =
                                               // chain_buffer[j];
  _func_exit_;
  return _SUCCESS;
...
u32 rtw_aes_encrypt(_adapter *padapter, u8 *pxmitframe) { // exclude ICV
  ...
  if ((pattrib->encrypt == _AES_)) {
    ...
    */
    ...
    /*
                            ...
                            RT_TRACE(_module_rtl871x_security_c_,_drv_err_,("rtw_aes_encrypt:
       stainfo==NULL!!!\n"));
                            DBG_871X("%s, psta==NUL\n", __func__);
                            res=_FAIL;
                    }
...
static sint aes_decipher(u8 *key, uint hdrlen, u8 *pframe, uint plen) {
  ...
  for (i = 0; i < num_blocks; i++) {
    construct_ctr_preload(
        ctr_preload, a4_exists, qc_exists, pframe, pn_vector, i + 1,
        frtype // add for CONFIG_IEEE80211W, none 11w also can use
        );
  ...
  {                          /* encrypt it and copy the unpadded part back   */
    construct_ctr_preload(
        ctr_preload, a4_exists, qc_exists, pframe, pn_vector, num_blocks + 1,
        frtype // add for CONFIG_IEEE80211W, none 11w also can use
        );
  ...
  if ((hdrlen + plen + 8) <= MAX_MSG_SIZE)
    _rtw_memcpy((void *)message, pframe,
  ...
  for (i = 0; i < num_blocks; i++) {
    construct_ctr_preload(
        ctr_preload, a4_exists, qc_exists, message, pn_vector, i + 1,
        frtype); // add for CONFIG_IEEE80211W, none 11w also can use
    aes128k128d(key, ctr_preload, aes_out);
    bitwise_xor(aes_out, &message[payload_index], chain_buffer);
    for (j = 0; j < 16; j++)
  ...
  {                          /* encrypt it and copy the unpadded part back   */
    construct_ctr_preload(
        ctr_preload, a4_exists, qc_exists, message, pn_vector, num_blocks + 1,
        frtype); // add for CONFIG_IEEE80211W, none 11w also can use
  ...
  /* Encrypt the MIC */
  construct_ctr_preload(
      ctr_preload, a4_exists, qc_exists, message, pn_vector, 0,
      frtype); // add for CONFIG_IEEE80211W, none 11w also can use
  ...
  for (i = 0; i < 8; i++) {
    if (pframe[hdrlen + 8 + plen - 8 + i] !=
      ...
      RT_TRACE(
          _module_rtl871x_security_c_, _drv_err_,
...
u32 rtw_aes_decrypt(_adapter *padapter, u8 *precvframe) { // exclude ICV
  ...
  if ((prxattrib->encrypt == _AES_)) {
    ...
    if (stainfo != NULL) {
      ...
      if(0)
      printk("@@@@@@@@@@@@@@@@@@ length=%d, prxattrib->hdrlen=%d,
      prxattrib->pkt_len=%d \n"
...
u32 rtw_BIP_verify(_adapter *padapter, u8 *precvframe) {
  ...
  _rtw_memcpy(BIP_AAD + BIP_AAD_SIZE, pframe + WLAN_HDR_A3_LEN,
  ...
  if (p) {
    ...
    if (omac1_aes_128(padapter->securitypriv
                          .dot11wBIPKey[padapter->securitypriv.dot11wBIPKeyid]
                          .skey,
                      BIP_AAD, ori_len, mic))
      goto BIP_exit;
...
static void rijndaelKeySetupEnc(u32 rk[/*44*/], const u8 cipherKey[]) {
  ...
  for (i = 0; i < 10; i++) {
    temp = rk[3];
    rk[4] =
        rk[0] ^ TE421(temp) ^ TE432(temp) ^ TE443(temp) ^ TE414(temp) ^ RCON(i);
    rk[5] = rk[1] ^ rk[4];
    rk[6] = rk[2] ^ rk[5];
    rk[7] = rk[3] ^ rk[6];
    rk += 4;
...
int wpa_tdls_ftie_mic(u8 *kck, u8 trans_seq, u8 *lnkid, u8 *rsnie,
  ...
  int ret;
  int len = 2 * ETH_ALEN + 1 + 2 + lnkid[1] + 2 + rsnie[1] + 2 + timeoutie[1] +
            2 + ftie[1];
  buf = rtw_zmalloc(len);
  if (!buf) {
...
int tdls_verify_mic(u8 *kck, u8 trans_seq, u8 *lnkid, u8 *rsnie, u8 *timeoutie,
                    u8 *ftie) {
  u8 *buf, *pos;
  int len;
  u8 mic[16];
...
void rtw_use_tkipkey_handler(RTW_TIMER_HDL_ARGS) {
  ...
  /*
     ...
     (padapter->bDriverStopped %d)(padapter->bSurpriseRemoved
...
u8 rtw_handle_tkip_countermeasure(_adapter *adapter, const char *caller) {
  ...
  if (securitypriv->btkip_countermeasure == _TRUE) {
    u32 passing_ms =
        rtw_get_passing_time_ms(securitypriv->btkip_countermeasure_time);
    if (passing_ms > 60 * 1000) {
      DBG_871X_LEVEL(_drv_always_,
                     "%s(" ADPT_FMT ") countermeasure time:%ds > 60s \n",
